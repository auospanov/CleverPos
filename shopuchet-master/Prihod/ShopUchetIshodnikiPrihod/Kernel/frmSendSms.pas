...
type
  TSendSmsForm = class(TDlgForm)
    ...
    procedure spReadCalcFields(DataSet: TDataSet);
...
begin
  ...
  inherited;
  aRefresh.Execute;
  tvClient.RestoreFromIniFile('Settings.ini', False, False, [], Self.Name + '-' + tvClient.Name);
...
procedure TSendSmsForm.spReadCalcFields(DataSet: TDataSet);
var str : string;
begin
  str := spRead.FieldByName('name').AsString;

  if cbTranslit.Checked then
    str := TranslitRus(str);

  spRead.FieldByName('smstext').AsString := StringREPLACE(mText.text, '@name@', str, []);
end;
