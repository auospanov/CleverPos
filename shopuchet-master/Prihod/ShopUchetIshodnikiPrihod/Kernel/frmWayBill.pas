...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      leIspolnitel.EditValue := ParamByName('OTVETST_ZA_POST_').asInteger;
      edTransp.Text := ParamByName('TRANSP_ORG_').AsString;
      edTTN.Text := ParamByName('TRANSP_NAKLAD_').AsString;
      leOtpuskAgree.EditValue := ParamByName('G_OFFICIAL_RUKOVOD_').asInteger;
      leOtpustil.EditValue := ParamByName('G_OFFICIAL_').asInteger;
...
begin
  with SP do begin
    ...
    if leIspolnitel.EditValue > 0 then
      ParamByName('OTVETST_ZA_POST_').asInteger := leIspolnitel.EditValue;
    ...
    if leOtpuskAgree.EditValue > 0 then
      ParamByName('G_OFFICIAL_RUKOVOD_').asInteger := leOtpuskAgree.EditValue;
    if leOtpustil.EditValue > 0 then
      ParamByName('G_OFFICIAL_').asInteger := leOtpustil.EditValue;
