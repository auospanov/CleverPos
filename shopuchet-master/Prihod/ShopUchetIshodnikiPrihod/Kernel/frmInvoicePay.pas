...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      spAddressClient.ParamByName('g_client_').AsInteger := ParamByName('G_CLIENT_ID_').AsInteger;
      OpenSP(spAddressClient, False);
      spAddressClient.First;
      leAddressClient.EditValue := ParamByName('ADDRESS_POKUPATEL_').AsInteger;
...
begin
  ...
  with SP do begin
    ...
    IF (leAddressClient.EditValue <> 0) and (leAddressClient.EditValue <> null) THEN
      ParamByName('ADDRESS_POKUPATEL_').AsInteger := leAddressClient.EditValue;
...
begin
  ...
  ReportsDM.spReport.ParamByName('INVOICE_PAY_').AsInteger := RecId;
