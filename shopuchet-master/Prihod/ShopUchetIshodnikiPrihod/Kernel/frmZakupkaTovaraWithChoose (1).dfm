inherited ZakupkaTovaraForm: TZakupkaTovaraForm
  Left = 476
  Top = 96
  BorderIcons = [biSystemMenu]
  Caption = #1055#1088#1080#1093#1086#1076' '#1090#1086#1074#1072#1088#1072
  ClientHeight = 636
  ClientWidth = 730
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 14
  inherited btnOk: TBitBtn
    Left = 471
    Top = 604
    Width = 166
    Caption = #1044#1086#1073#1072#1074#1080#1090#1100' '#1074' '#1085#1072#1082#1083#1072#1076#1085#1091#1102
  end
  inherited btnCancel: TBitBtn
    Left = 643
    Top = 604
    Caption = #1047#1072#1082#1088#1099#1090#1100
  end
  inherited Panel1: TPanel
    Width = 718
    Height = 591
    object Panel2: TPanel
      Left = 2
      Top = 332
      Width = 714
      Height = 257
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object Shape19: TShape
        Left = 390
        Top = 109
        Width = 129
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape17: TShape
        Left = 390
        Top = 84
        Width = 129
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape14: TShape
        Left = 390
        Top = 59
        Width = 129
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape2: TShape
        Left = 390
        Top = 34
        Width = 129
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape4: TShape
        Left = 390
        Top = 9
        Width = 129
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape7: TShape
        Left = 7
        Top = 59
        Width = 146
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape5: TShape
        Left = 7
        Top = 34
        Width = 146
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape1: TShape
        Left = 7
        Top = 9
        Width = 146
        Height = 26
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Label1: TLabel
        Left = 17
        Top = 15
        Width = 67
        Height = 14
        Caption = #1050#1086#1083#1080#1095#1077#1089#1090#1074#1086
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape3: TShape
        Left = 152
        Top = 9
        Width = 218
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label3: TLabel
        Left = 17
        Top = 40
        Width = 28
        Height = 14
        Caption = #1062#1077#1085#1072
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape6: TShape
        Left = 152
        Top = 34
        Width = 218
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label4: TLabel
        Left = 17
        Top = 65
        Width = 35
        Height = 14
        Caption = #1057#1091#1084#1084#1072
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape8: TShape
        Left = 152
        Top = 59
        Width = 218
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label2: TLabel
        Left = 396
        Top = 15
        Width = 46
        Height = 14
        Caption = #1054#1089#1090#1072#1090#1086#1082
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Label7: TLabel
        Left = 396
        Top = 40
        Width = 117
        Height = 14
        Caption = #1062#1077#1085#1072' '#1087#1088#1086#1076#1072#1078#1080' '#1088#1086#1079#1085'.'
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape12: TShape
        Left = 518
        Top = 9
        Width = 190
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Shape13: TShape
        Left = 518
        Top = 34
        Width = 190
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label8: TLabel
        Left = 396
        Top = 65
        Width = 107
        Height = 14
        Caption = #1062#1077#1085#1072' '#1087#1088#1086#1076#1072#1078#1080' '#1086#1087#1090
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape16: TShape
        Left = 518
        Top = 59
        Width = 190
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label9: TLabel
        Left = 396
        Top = 90
        Width = 107
        Height = 14
        Caption = #1053#1072#1094#1077#1085#1082#1072' '#1088#1086#1079#1085'. (%)'
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape18: TShape
        Left = 518
        Top = 84
        Width = 190
        Height = 26
        Pen.Color = clBtnShadow
      end
      object Label11: TLabel
        Left = 396
        Top = 115
        Width = 97
        Height = 14
        Caption = #1053#1072#1094#1077#1085#1082#1072' '#1086#1087#1090' (%)'
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape20: TShape
        Left = 518
        Top = 109
        Width = 190
        Height = 26
        Pen.Color = clBtnShadow
      end
      object GroupBox1: TGroupBox
        Left = 8
        Top = 89
        Width = 362
        Height = 75
        Caption = #1042' '#1090#1086#1084' '#1095#1080#1089#1083#1077' '#1053#1044#1057
        TabOrder = 2
        object Shape10: TShape
          Left = 7
          Top = 42
          Width = 96
          Height = 26
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape9: TShape
          Left = 7
          Top = 17
          Width = 97
          Height = 26
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Label5: TLabel
          Left = 17
          Top = 23
          Width = 59
          Height = 14
          Caption = #1057#1090#1072#1074#1082#1072'(%)'
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Shape15: TShape
          Left = 102
          Top = 17
          Width = 138
          Height = 26
          Pen.Color = clBtnShadow
        end
        object Label6: TLabel
          Left = 17
          Top = 47
          Width = 35
          Height = 14
          Caption = #1057#1091#1084#1084#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Shape11: TShape
          Left = 102
          Top = 42
          Width = 138
          Height = 26
          Pen.Color = clBtnShadow
        end
        object edNDS: TVLSummdxEdit
          Left = 110
          Top = 19
          Width = 123
          AddTrailingZeros = True
          SummValueStr = '0.00'
          SummValue = '0.00'
          Alignment = taRightJustify
          AutoSelect = False
          TabOrder = 0
          Text = '0.00'
          OnChange = edNDSChange
          StoredValues = 1
        end
        object edNDSSumm: TdxCurrencyEdit
          Left = 110
          Top = 45
          Width = 123
          Enabled = False
          TabOrder = 1
          Alignment = taRightJustify
          DisplayFormat = ',0.00;-,0.00'
          StoredValues = 1
        end
      end
      object edPrice: TdxCurrencyEdit
        Left = 157
        Top = 36
        Width = 208
        TabOrder = 1
        Alignment = taRightJustify
        OnChange = edPriceChange
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object edSumm: TdxCurrencyEdit
        Left = 157
        Top = 61
        Width = 208
        Enabled = False
        TabOrder = 3
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      inline MyCurrencyFrameForm: TMyCurrencyFrameForm
        Left = 158
        Top = 12
        Width = 206
        Height = 21
        TabOrder = 0
        inherited edAmount: TEdit
          Top = -1
          Width = 186
          Height = 22
          OnChange = MyCurrencyFrameForm1edAmountChange
        end
        inherited sbCur: TSpinButton
          Left = 185
        end
      end
      object sePriceOpt: TdxCurrencyEdit
        Left = 527
        Top = 61
        Width = 178
        Enabled = False
        TabOrder = 4
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object edNakrutkaR: TdxCurrencyEdit
        Left = 527
        Top = 86
        Width = 178
        Enabled = False
        TabOrder = 5
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object edNakrutkaO: TdxCurrencyEdit
        Left = 527
        Top = 111
        Width = 178
        Enabled = False
        TabOrder = 6
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object seAmount: TdxCurrencyEdit
        Left = 527
        Top = 12
        Width = 178
        Enabled = False
        TabOrder = 7
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object sePrice: TdxCurrencyEdit
        Left = 527
        Top = 36
        Width = 178
        Enabled = False
        TabOrder = 8
        Alignment = taRightJustify
        DisplayFormat = ',0.00;-,0.00'
        StoredValues = 1
      end
      object Panel5: TPanel
        Left = 0
        Top = 197
        Width = 714
        Height = 61
        BevelInner = bvRaised
        BevelOuter = bvLowered
        TabOrder = 9
        object Shape23: TShape
          Left = 482
          Top = 33
          Width = 88
          Height = 26
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape21: TShape
          Left = 482
          Top = 8
          Width = 88
          Height = 26
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Label12: TLabel
          Left = 488
          Top = 14
          Width = 61
          Height = 14
          Caption = #1056#1086#1079#1085'. '#1094#1077#1085#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Shape22: TShape
          Left = 569
          Top = 8
          Width = 140
          Height = 26
          Pen.Color = clBtnShadow
        end
        object Label13: TLabel
          Left = 488
          Top = 38
          Width = 57
          Height = 14
          Caption = #1054#1087#1090'. '#1094#1077#1085#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Shape24: TShape
          Left = 569
          Top = 33
          Width = 140
          Height = 26
          Pen.Color = clBtnShadow
        end
        object cbUpdatePrice: TCheckBox
          Left = 9
          Top = 12
          Width = 124
          Height = 17
          Hint = 
            #1055#1086#1089#1090#1072#1074#1100#1090#1077' '#1075#1072#1083#1086#1095#1082#1091', '#1077#1089#1083#1080' '#1093#1086#1090#1080#1090#1077' '#1080#1079#1084#1077#1085#1080#1090#1100' '#1094#1077#1085#1091' '#1090#1086#1074#1072#1088#1072' '#1074' '#1072#1074#1090#1086#1084#1072#1090#1080#1095#1077 +
            #1089#1082#1086#1084' '#1088#1077#1078#1080#1084#1077'. '#1062#1077#1085#1072' '#1080#1079#1084#1077#1085#1080#1090#1089#1103' '#1087#1086' '#1085#1072#1078#1072#1090#1080#1102' '#1085#1072' '#1082#1085#1086#1087#1082#1091' "'#1044#1086#1073#1072#1074#1080#1090#1100' '#1074' '#1085#1072#1082 +
            #1083#1072#1076#1085#1091#1102'"'
          Caption = #1088#1086#1079#1085'. '#1085#1072#1094#1077#1085#1082#1072'  '#1085#1072' '
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
          OnClick = cbUpdatePriceClick
        end
        object edNacenka: TdxCurrencyEdit
          Left = 135
          Top = 10
          Width = 70
          Enabled = False
          TabOrder = 1
          Alignment = taRightJustify
          OnChange = edNacenkaChange
          DisplayFormat = ',0.00%'
          StoredValues = 1
        end
        object edNewPrice: TdxCurrencyEdit
          Left = 573
          Top = 10
          Width = 132
          Enabled = False
          TabOrder = 2
          Alignment = taRightJustify
          OnChange = edPriceChange
          DisplayFormat = ',0.00;-,0.00'
          StoredValues = 1
        end
        object cbProcOrVal: TComboBox
          Left = 209
          Top = 10
          Width = 51
          Height = 22
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 3
          Text = #1087#1088#1086#1094
          Items.Strings = (
            #1087#1088#1086#1094
            #1074#1072#1083)
        end
        object cbPrice: TComboBox
          Left = 265
          Top = 10
          Width = 162
          Height = 22
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 4
          Text = #1086#1090' '#1090#1077#1082#1091#1097#1077#1081' '#1094#1077#1085#1099' '#1079#1072#1082#1091#1087#1072
          Items.Strings = (
            #1086#1090' '#1090#1077#1082#1091#1097#1077#1081' '#1094#1077#1085#1099' '#1079#1072#1082#1091#1087#1072
            #1086#1090' '#1087#1086#1089#1083#1077#1076#1085#1080#1093' N '#1079#1072#1082#1091#1087#1086#1082)
        end
        object edZakupAmount: TdxCurrencyEdit
          Left = 430
          Top = 10
          Width = 30
          Enabled = False
          TabOrder = 5
          Alignment = taRightJustify
          OnChange = edPriceChange
          DecimalPlaces = 0
          Value = 5.000000000000000000
          StoredValues = 1
        end
        object cbUpdateOptPrice: TCheckBox
          Left = 9
          Top = 36
          Width = 124
          Height = 17
          Hint = 
            #1055#1086#1089#1090#1072#1074#1100#1090#1077' '#1075#1072#1083#1086#1095#1082#1091', '#1077#1089#1083#1080' '#1093#1086#1090#1080#1090#1077' '#1080#1079#1084#1077#1085#1080#1090#1100' '#1094#1077#1085#1091' '#1090#1086#1074#1072#1088#1072' '#1074' '#1072#1074#1090#1086#1084#1072#1090#1080#1095#1077 +
            #1089#1082#1086#1084' '#1088#1077#1078#1080#1084#1077'. '#1062#1077#1085#1072' '#1080#1079#1084#1077#1085#1080#1090#1089#1103' '#1087#1086' '#1085#1072#1078#1072#1090#1080#1102' '#1085#1072' '#1082#1085#1086#1087#1082#1091' "'#1044#1086#1073#1072#1074#1080#1090#1100' '#1074' '#1085#1072#1082 +
            #1083#1072#1076#1085#1091#1102'"'
          Caption = #1086#1087#1090'. '#1085#1072#1094#1077#1085#1082#1072'  '#1085#1072' '
          ParentShowHint = False
          ShowHint = True
          TabOrder = 6
          OnClick = cbUpdatePriceClick
        end
        object edOptNacenka: TdxCurrencyEdit
          Left = 135
          Top = 34
          Width = 70
          Enabled = False
          TabOrder = 7
          Alignment = taRightJustify
          OnChange = edNacenkaChange
          DisplayFormat = ',0.00%'
          StoredValues = 1
        end
        object cbProcOrValOpt: TComboBox
          Left = 209
          Top = 34
          Width = 51
          Height = 22
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 8
          Text = #1087#1088#1086#1094
          Items.Strings = (
            #1087#1088#1086#1094
            #1074#1072#1083)
        end
        object cbPriceOpt: TComboBox
          Left = 265
          Top = 34
          Width = 162
          Height = 22
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 9
          Text = #1086#1090' '#1090#1077#1082#1091#1097#1077#1081' '#1094#1077#1085#1099' '#1079#1072#1082#1091#1087#1072
          Items.Strings = (
            #1086#1090' '#1090#1077#1082#1091#1097#1077#1081' '#1094#1077#1085#1099' '#1079#1072#1082#1091#1087#1072
            #1086#1090' '#1087#1086#1089#1083#1077#1076#1085#1080#1093' N '#1079#1072#1082#1091#1087#1086#1082)
        end
        object edZakupAmountOpt: TdxCurrencyEdit
          Left = 430
          Top = 34
          Width = 30
          Enabled = False
          TabOrder = 10
          Alignment = taRightJustify
          OnChange = edPriceChange
          DecimalPlaces = 0
          Value = 5.000000000000000000
          StoredValues = 1
        end
        object edNewPriceOpt: TdxCurrencyEdit
          Left = 573
          Top = 35
          Width = 132
          Enabled = False
          TabOrder = 11
          Alignment = taRightJustify
          OnChange = edPriceChange
          DisplayFormat = ',0.00;-,0.00'
          StoredValues = 1
        end
      end
      object cbShowMode: TCheckBox
        Left = 8
        Top = 176
        Width = 354
        Height = 17
        Caption = #1055#1086#1082#1072#1079#1072#1090#1100' '#1088#1077#1078#1080#1084' '#1072#1074#1090#1086#1084#1072#1090#1080#1095#1077#1089#1082#1086#1075#1086' '#1087#1077#1088#1077#1089#1095#1077#1090#1072' '#1094#1077#1085#1099' '#1090#1086#1074#1072#1088#1072
        TabOrder = 10
        OnClick = cbShowModeClick
      end
    end
    object Panel3: TPanel
      Left = 2
      Top = 2
      Width = 714
      Height = 330
      Align = alTop
      Caption = 'Panel3'
      TabOrder = 0
      object Panel4: TPanel
        Left = 1
        Top = 1
        Width = 712
        Height = 37
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 0
        object Label10: TLabel
          Left = 8
          Top = 9
          Width = 40
          Height = 16
          Caption = #1055#1086#1080#1089#1082
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object sbClear: TSpeedButton
          Left = 277
          Top = 8
          Width = 26
          Height = 22
          Hint = #1054#1095#1080#1089#1090#1080#1090#1100' '#1087#1086#1080#1089#1082
          Glyph.Data = {
            
            FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
          ParentShowHint = False
          ShowHint = True
          OnClick = sbClearClick
        end
        object edBarcode: TEdit
          Left = 54
          Top = 8
          Width = 217
          Height = 22
          MaxLength = 250
          TabOrder = 0
          OnChange = edBarcodeChange
        end
        object btIns: TBitBtn
          Left = 518
          Top = 7
          Width = 107
          Height = 25
          Action = aIns
          Caption = #1053#1086#1074#1099#1081' '#1090#1086#1074#1072#1088
          TabOrder = 1
          Glyph.Data = {
            
            9200E0C8B300FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        end
        object btUpd: TBitBtn
          Left = 624
          Top = 7
          Width = 83
          Height = 25
          Action = aUpd
          Caption = #1048#1079#1084#1077#1085#1080#1090#1100
          TabOrder = 2
          Glyph.Data = {
            
            FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        end
      end
      object tlCategory: TdxDBTreeList
        Left = 1
        Top = 38
        Width = 250
        Height = 291
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'G_PRODUCT'
        ParentField = 'G_PRODUCT_PAR'
        Align = alLeft
        TabOrder = 1
        DataSource = dsReadCategory
        OptionsBehavior = [etoAutoDragDrop, etoAutoDragDropCopy, etoDragExpand, etoDragScroll, etoEditing, etoEnterShowEditor, etoImmediateEditor, etoStoreToRegistry, etoTabThrough]
        RegistryPath = '\Software\ShopUchet\ZakupkaTovara\tlCategory'
        TreeLineColor = clGrayText
        OnChangeNode = tlCategoryChangeNode
        object tlCategoryG_PRODUCT: TdxDBTreeListMaskColumn
          Visible = False
          Width = 130
          BandIndex = 0
          RowIndex = 0
          FieldName = 'G_PRODUCT'
        end
        object tlCategoryG_PRODUCT_PAR: TdxDBTreeListMaskColumn
          Visible = False
          Width = 180
          BandIndex = 0
          RowIndex = 0
          FieldName = 'G_PRODUCT_PAR'
        end
        object tlCategoryNAME: TdxDBTreeListMaskColumn
          Caption = #1050#1072#1090#1077#1075#1086#1088#1080#1103
          Width = 200
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NAME'
        end
      end
      object splMenu: TcxSplitter
        Left = 251
        Top = 38
        Width = 8
        Height = 291
        HotZoneClassName = 'TcxSimpleStyle'
        HotZone.SizePercent = 32
        AutoPosition = False
        PositionAfterOpen = 150
        AutoSnap = True
        ResizeUpdate = True
        Control = tlCategory
        Color = clBtnFace
        ParentColor = False
      end
      object grProduct: TdxDBGrid
        Left = 259
        Top = 38
        Width = 454
        Height = 291
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'G_PRODUCT'
        SummaryGroups = <>
        SummarySeparator = ', '
        Align = alClient
        TabOrder = 3
        OnDblClick = grProductDblClick
        DataSource = dsRead
        Filter.Criteria = {00000000}
        OptionsBehavior = [edgoAutoSearch, edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoMouseScroll, edgoMultiSelect, edgoMultiSort, edgoStoreToRegistry, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
        RegistryPath = '\Software\ShopUchet\ZakupkaTovara\grProduct'
        OnChangeNode = grProductChangeNode
        object dxDBGridMaskColumn1: TdxDBGridMaskColumn
          Alignment = taCenter
          HeaderAlignment = taCenter
          Visible = False
          Width = 90
          BandIndex = 0
          RowIndex = 0
          FieldName = 'G_PRODUCT'
        end
        object dxDBGridMaskColumn2: TdxDBGridMaskColumn
          Alignment = taCenter
          HeaderAlignment = taCenter
          Visible = False
          Width = 90
          BandIndex = 0
          RowIndex = 0
          FieldName = 'G_PRODUCT_PAR'
        end
        object dxDBGridMaskColumn6: TdxDBGridMaskColumn
          Alignment = taLeftJustify
          Caption = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
          HeaderAlignment = taCenter
          Width = 178
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NAME'
        end
        object dxDBGridMaskColumn3: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #1040#1088#1090#1080#1082#1091#1083
          HeaderAlignment = taCenter
          Width = 75
          BandIndex = 0
          RowIndex = 0
          FieldName = 'ARTICUL'
        end
        object dxDBGridMaskColumn4: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #1053#1086#1084#1077#1085#1082#1083'. '#1085#1086#1084#1077#1088
          HeaderAlignment = taCenter
          Width = 117
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NOMENCLATUR_NUM'
        end
        object dxDBGridMaskColumn5: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #1064#1090#1088#1080#1093#1082#1086#1076
          HeaderAlignment = taCenter
          Width = 79
          BandIndex = 0
          RowIndex = 0
          FieldName = 'BARCODE'
        end
      end
    end
  end
  object cbClose: TCheckBox [3]
    Left = 8
    Top = 605
    Width = 241
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = #1047#1072#1082#1088#1099#1074#1072#1090#1100' '#1086#1082#1085#1086' '#1087#1086#1089#1083#1077' '#1076#1086#1073#1072#1074#1083#1077#1085#1080#1103' '#1090#1086#1074#1072#1088#1072' '#1074' '#1085#1072#1082#1083#1072#1076#1085#1091#1102
    TabOrder = 4
    WordWrap = True
  end
  inherited btHelp: TBitBtn
    Left = 272
    Top = 604
  end
  inherited dxEditStyleController: TdxEditStyleController
    Left = 112
    Top = 664
  end
  inherited dxCheckEditStyleController: TdxCheckEditStyleController
    Left = 144
    Top = 664
  end
  inherited ActionList: TActionList
    Images = MainDM.imMain
    Left = 176
    Top = 664
    object aChooseTovar: TAction
      Caption = #1042#1099#1073#1088#1072#1090#1100
      OnExecute = aChooseTovarExecute
    end
    object aRefreshChd: TAction
      Caption = 'aRefreshChd'
      OnExecute = aRefreshChdExecute
    end
    object aIns: TAction
      Caption = #1053#1086#1074#1099#1081' '#1090#1086#1074#1072#1088
      ImageIndex = 150
      ShortCut = 45
      OnExecute = aInsExecute
    end
    object aUpd: TAction
      Caption = #1048#1079#1084#1077#1085#1080#1090#1100
      ImageIndex = 148
      OnExecute = aUpdExecute
    end
  end
  inherited tranRead: TpFIBTransaction
    Left = 152
    Top = 608
  end
  inherited spGetData: TpFIBStoredProc
    Left = 40
    Top = 608
  end
  inherited tranWrite: TpFIBTransaction
    Left = 184
    Top = 608
  end
  inherited spIns: TpFIBStoredProc
    SQL.Strings = (
      
        'EXECUTE PROCEDURE INS_SKLAD_TMP (?G_PRODUCT_, ?AMOUNT_, ?PRICE_,' +
        ' ?SUMM_, ?NDS_, ?NDS_SUMM_)')
    StoredProcName = 'INS_SKLAD_TMP'
    Left = 72
    Top = 608
  end
  inherited spUpd: TpFIBStoredProc
    Left = 104
    Top = 608
  end
  object dsRead: TDataSource
    DataSet = spRead
    Left = 320
    Top = 152
  end
  object dsReadCategory: TDataSource
    DataSet = spReadCategory
    Left = 56
    Top = 104
  end
  object spReadCategory: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_PRODCAT_LIST')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 24
    Top = 104
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spRead: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT'
      '        T.G_PRODUCT,'
      '        T.G_PRODUCT_PAR,'
      '        T.ARTICUL,'
      '        T.NOMENCLATUR_NUM,'
      '        T.BARCODE,'
      '        T.NAME,'
      '        T.PRICE,'
      '        T.VOLUME_PRICE,'
      '        COALESCE(P.AMOUNT, 0) AMOUNT'
      '    FROM (SELECT DISTINCT(SS.G_PRODUCT)'
      '          FROM (WITH RECURSIVE TREE'
      '                AS (SELECT G.G_PRODUCT'
      '                      FROM G_PRODUCT G'
      
        '                     WHERE G.G_PRODUCT = DECODE(:G_PRODUCT_PAR_,' +
        ' -1, G.G_PRODUCT, :G_PRODUCT_PAR_)'
      '                       AND G.IS_CATEGORY = 1'
      '                    UNION ALL'
      '                    SELECT G.G_PRODUCT'
      
        '                      FROM G_PRODUCT G INNER JOIN TREE PRIOR ON ' +
        'PRIOR.G_PRODUCT = G.G_PRODUCT_PAR'
      '                     WHERE G.IS_CATEGORY = 1'
      '                  )'
      '               SELECT * FROM TREE) SS) S1,'
      '          (SELECT G_PRODUCT,'
      '                 G_PRODUCT_PAR,'
      '                 ARTICUL,'
      '                 NOMENCLATUR_NUM,'
      '                 BARCODE,'
      '                 NAME,'
      '                 PRICE,'
      '                 VOLUME_PRICE'
      '            FROM G_PRODUCT'
      '           WHERE IS_CATEGORY = 0'
      '             AND IS_ACTIVE = 1'
      '          UNION ALL'
      '          SELECT T1.G_PRODUCT,'
      '                 T1.G_PRODUCT_PAR,'
      '                 T1.ARTICUL,'
      '                 T1.NOMENCLATUR_NUM,'
      '                 B.BARCODE,'
      '                 T1.NAME||'#39' ('#1076#1086#1087'.'#1096#1090#1088#1080#1093#1082#1086#1076')'#39','
      '                 T1.PRICE,'
      '                 T1.VOLUME_PRICE'
      '            FROM G_PRODUCT T1, LT_BARCODE B'
      '           WHERE IS_CATEGORY = 0'
      '             AND IS_ACTIVE = 1'
      
        '             AND B.G_PRODUCT = T1.G_PRODUCT) T LEFT OUTER JOIN (' +
        'SELECT * FROM LT_SKLAD_PRODUCT WHERE G_TOCHKA = :G_TOCHKA_)  P O' +
        'N T.G_PRODUCT = P.G_PRODUCT'
      '    WHERE T.G_PRODUCT_PAR = S1.G_PRODUCT'
      '    ORDER BY 6'
      ' '
      ' ')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 288
    Top = 152
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spFindProduct: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      
        'EXECUTE PROCEDURE G_GET_G_PRODUCT_BY_BARCODE (?BARCODE_, ?ARTICU' +
        'L_, ?NOMER_, ?NAME_)')
    StoredProcName = 'G_GET_G_PRODUCT_BY_BARCODE'
    Left = 368
    Top = 144
  end
  object aeScanner: TApplicationEvents
    OnMessage = aeScannerMessage
    Left = 168
    Top = 160
  end
  object TimerChangeEd: TTimer
    Enabled = False
    Interval = 500
    OnTimer = TimerChangeEdTimer
    Left = 432
    Top = 144
  end
  object GetLastPrice: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      
        'EXECUTE PROCEDURE GET_LAST_SELF_PRICE (?G_PRODUCT_, ?G_TOCHKA_, ' +
        '?MODE_)')
    StoredProcName = 'GET_LAST_SELF_PRICE'
    Left = 344
    Top = 395
  end
end
