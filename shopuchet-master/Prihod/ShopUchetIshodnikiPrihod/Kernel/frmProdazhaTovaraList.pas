...
type
  TProdazhaTovaraListForm = class(TDBListForm)
    ...
    fbcdfldReadSUMM_CASH: TFIBBCDField;
    fbcdfldReadSUMM_NONCASH: TFIBBCDField;
    aPrintSimple: TAction;
    dxBarSubItem2: TdxBarSubItem;
    dxbrbtn3: TdxBarButton;
    clmMainG_OFFICIAL_FIO: TcxGridDBColumn;
    fbntgrfldReadG_OFFICIAL: TFIBIntegerField;
    fbstrngfldReadG_OFFICIAL_FIO: TFIBStringField;
    aDoReturn: TAction;
...
uses frmProdazhaTovaraN, frmInvoicePay, frmInvoice, frmWayBill, dmReports, frmDoc, frmPrihodTovaraN, unErrorHandlers;
  ...
  IF FMode IN [1, 3, 4] THEN BEGIN
    ...
    if FMode IN [1, 3, 4] THEN BEGIN
      ...
      if FMode in [1,4] then
        ...
        aPrintSimple.Visible := False;
        aWayBill.Caption := 'Накладная';
      end;
    END;
...
begin
  ...
  try
    ...
    if spRead.FieldByName('INVOICE_PAY').AsString <> '' then
      ProdazhaTovaraNForm.InvoicePay := spRead.FieldByName('INVOICE_PAY').AsInteger;
...
begin
  ...
  try
    ...
    PrihodTovaraNForm.Zakaz := spRead.FieldByName('zakaz').AsInteger;
...
begin
  ...
  ReportsDM.spReport.ParamByName('ZAKAZ_').AsInteger := spRead.FieldByName('ZAKAZ').AsInteger;
...
begin
  ...
  tvDetails.StoreToIniFile('Settings.ini', False, [], Self.Name + '-' + tvDetails.name);
end;
