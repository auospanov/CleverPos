...
begin
  ...
  if leTovarCategory.EditValue = 0 then begin
    ErrControl := edName;
  ...
  if (edSkidkaSum.Value = 0) AND (peTovar.Tag <> 0) then begin
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      leTovarCategory.EditValue := ParamByName('G_PRODUCT_PAR_').AsInteger;
      peTovar.Tag := ParamByName('G_PRODUCT_').AsInteger;
...
begin
  ...
  with SP do begin
    ...
    ParamByName('G_PRODUCT_par_').AsInteger := leTovarCategory.EditValue;
    if peTovar.Tag <> 0 then
...
begin
  ...
  if (leTovarCategory.EditValue <> 0) and (leTovarCategory.EditValue <> null) then
    FPopupTovarForm.spRead.ParamByName('g_product_par_').AsInteger := leTovarCategory.EditValue;
  FPopupTovarForm.arefresh.Execute;
end;

procedure TDiscountForm.ChangeFields;
begin
  if (peTovar.Tag <> 0) AND (ceProcOrTenge.Checked = FALSE) then
    edSkidkaSum.Value := edPrice.Value - edPrice.Value * (1 - seSkidka.Value/100)

  else if (peTovar.Tag > 0) AND (ceProcOrTenge.Checked) then
