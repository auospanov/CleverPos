...
type
  TPriceListUnForm = class(TDBEditForm)
    ...
    procedure cbTovarChange(Sender: TObject);
    procedure cbTovarClick(Sender: TObject);
    procedure edMFCountryKeyPress(Sender: TObject; var Key: Char);
    procedure edPriceChange(Sender: TObject);
    procedure edVolPriceChange(Sender: TObject);
  ...
  private
    ...
    FG_PROD_FOR_NEW_PRICE : INTEGER;  // ФОРМА вызвана для создания старого товара с новой ценой, ПЕРЕДАЕТСЯ ID СТАРОГО ТОВАРА
  ...
  public
    ...
    property G_PROD_FOR_NEW_PRICE : Integer read FG_PROD_FOR_NEW_PRICE write FG_PROD_FOR_NEW_PRICE;  // ФОРМА вызвана для создания старого товара с новой ценой, ПЕРЕДАЕТСЯ ID СТАРОГО ТОВАРА
...
uses unCommonFunc, frmCategory, PriceListUnSostav;
...
begin
  ...
  if Config <> 3 then begin
    cbTovar.Items.Delete(3);
    cbTovar.Items.Delete(2);
  end;
  ...
  lbSostavProduct.Width := ScaleDimension(550);
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      edArticul.Text    := ParamByName('ARTICUL_').AsString;
      leGUnit.EditValue := ParamByName('G_UNIT_').AsInteger;
...
begin
  ...
  with SP do begin
    ...
    ParamByName('ARTICUL_').AsString  := edArticul.Text;
    ...
    ParamByName('G_UNIT_').AsInteger := leGUnit.EditValue;
    ...
    if Config = 2 then
      if lbCross.Items.Count > 0 then begin
        for i := 0 to lbCross.Items.Count - 1 do
...
begin
  if (cxPageControl.ActivePage = tsBarcode) and (spBarcode.Active = False) then
    aRefreshBarcode.Execute
  else if (cxPageControl.ActivePage = tsPhoto) and (spReadPics.Active = False) then begin
    ReadPictureList(False);
    if spReadPics.RecordCount > 0 then
      ShowPicture;
  end else if (cxPageControl.ActivePage = tsSostav) and {(spReadKit.Active = False) and} (cbTovar.ItemIndex > 0) then
    ReadSostav;
end;
...
begin
  ...
  if InputQuery( 'Новый штрихкод', 'Нажмите ОК или введите свой штрихкод', Barcode) then begin
    ...
    if ExecSPTR(spInsBarcode) then begin
      aRefreshBarcode.Execute;
      spBarcode.locate('lt_barcode', spInsBarcode.ParamByName('lt_barcode_').AsInteger, []);
...
begin
  spDelBarcode.ParamByName('LT_BARCODE_').AsInteger := spBarcode.FieldByName('lt_barcode').AsInteger;
...
begin
  ...
  spUpdatePics.Params[0].Value := spReadPics.FieldByName('G_PRODUCT_IMAGE').AsInteger;
  ExecQuery;
  ReadPictureList(True);
  IF spReadPics.RecordCount > 0 THEN
    ShowPicture
  ...
  with PriceListUnSostavForm do begin
    try
      ...
      if ShowModal = mrOk then begin
        if spReadKit.Locate('g_product', spRead.FieldByName('g_product').AsInteger, []) then begin
          ...
          spUpdatePics.ParamByName('G_PRODUCT_ITEM_').AsInteger := spRead.FieldByName('g_product').AsInteger;
          spUpdatePics.ParamByName('AMOUNT_').AsFloat := StrToFloat(MyCurrencyFrameForm.edAmount.Text) + spUpdatePics.ParamByName('AMOUNT_').AsFloat;
          ...
          spUpdatePics.ParamByName('G_PRODUCT_').AsInteger := RecID;
          spUpdatePics.ParamByName('G_PRODUCT_ITEM_').AsInteger := spRead.FieldByName('g_product').AsInteger;
          spUpdatePics.ParamByName('AMOUNT_').AsFloat := StrToFloat(MyCurrencyFrameForm.edAmount.Text);
          spUpdatePics.ParamByName('PRICE_').AsFloat := edPrice.Value;
          spUpdatePics.ParamByName('PRICE1_').AsFloat := edPriceVol.Value;
        end;
        ExecQuery;
...
begin
  ...
  with PriceListUnSostavForm do begin
    try
      Id := spReadKit.FieldByName('g_product').AsInteger;
      Typ := cbTovar.ItemIndex;
      ExceptId := RecId;
      Amount := spReadKit.FieldByName('amount').AsString;
      if ShowModal = mrOk then begin
        {mdSostav.Edit;
        mdSostav.FieldByName('price').asFloat := edPrice.Value;
        mdSostav.FieldByName('amount').AsFloat := StrToFloat(MyCurrencyFrameForm.edAmount.Text);
        mdSostav.Refresh;}
        ...
        spUpdatePics.ParamByName('G_PRODUCT_ITEM_').AsInteger := spReadKit.FieldByName('G_PRODUCT').AsInteger;
        spUpdatePics.ParamByName('AMOUNT_').AsFloat := StrToFloat(MyCurrencyFrameForm.edAmount.Text);
        spUpdatePics.ParamByName('PRICE_').AsFloat := edPrice.Value;
        spUpdatePics.ParamByName('PRICE1_').AsFloat := edPriceVol.Value;
        ExecQuery;
...
procedure TPriceListUnForm.cbTovarChange(Sender: TObject);
begin
  ReadSostav;
end;
...
begin
  ...
  lbSostavProduct.Visible := cbTovar.ItemIndex = 3;
  edPrice.Properties.ReadOnly := cbTovar.ItemIndex = 1;
  edVolPrice.Properties.ReadOnly := cbTovar.ItemIndex = 1;
  ...
  if spReadKit.RecordCount > 0 then begin
    edPrice.Value := 0;
    edVolPrice.Value := 0;
...
procedure TPriceListUnForm.cbTovarClick(Sender: TObject);
begin
  inherited;
  edName.ModifiedAfterEnter := True;
end;
...
procedure TPriceListUnForm.edPriceChange(Sender: TObject);
begin
...
procedure TPriceListUnForm.edVolPriceChange(Sender: TObject);
begin
