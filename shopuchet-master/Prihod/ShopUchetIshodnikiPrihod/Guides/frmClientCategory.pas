...
begin
  inherited;
  spClientCategory.ParamByName('G_CLIENT_').AsInteger := RecID;
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      leClientCategory.EditValue := ParamByName('g_client_parent_').AsInteger;
...
begin
  ...
  with SP do begin
    if leClientCategory.EditValue > 0 then
      ParamByName('G_CLIENT_PARENT_').AsInteger := leClientCategory.EditValue;
