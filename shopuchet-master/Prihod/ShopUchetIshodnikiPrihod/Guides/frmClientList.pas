...
begin
  ...
  aDel.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('G_CLIENT').AsInteger > 0);
  aView.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('G_CLIENT').AsInteger > 0);
  aInsChdCategory.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('G_CLIENT').AsInteger > 0);
  aUpd.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('G_CLIENT').AsInteger > 0);
  aInsLike.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('G_CLIENT').AsInteger > 0);
...
begin
  if pcClientRekv.ActivePage = tsContact then begin
    ...
    spClientHis.ParamByName('g_client_').AsInteger := spReadClient.FieldByName('G_client').AsInteger;
...
begin
  ...
  try
    if FIns_Child = 1 then
      (ClassForm as TClientCategoryForm).leClientCategory.EditValue := spRead.FieldByName('g_client').AsInteger
    else
...
begin
  ...
  inherited;
  tvClient.StoreToIniFile('Settings.ini', False, [], Self.Name + '-' + tvClient.name);
  tvBank.StoreToIniFile('Settings.ini', False, [], Self.Name + '-' + tvBank.name);
  tvHis.StoreToIniFile('Settings.ini', False, [], Self.Name + '-' + tvHis.name);
  tvAddress.StoreToIniFile('Settings.ini', False, [], Self.Name + '-' + tvAddress.name);
  SaveSettingsIni('ClientList', 'SplitterPos', pcClientRekv.Height, '');
end;
