...
uses
  ...
  cxContainer, cxTextEdit, cxMaskEdit, cxDropDownEdit, FIBQuery, pFIBQuery,
  pFIBStoredProc, cxNavigator, System.Actions, system.UITypes, cxButtons,
...
procedure TPopupListForm.ClosePopupForm(Sender : TObject);
var
...
procedure TPopupListForm.Deldata;
var
  ErrCode: Integer;
  ErrMsg : String;
begin
...
begin
  ...
  if (tvMain.Controller.FocusedRow <> Nil) then begin
    if FSelectedFieldIsStr then
      ...
      FSelectedID := spRead.FieldByName('ID').AsInteger;
    AcceptRes := True;
  end;
...
procedure TPopupListForm.tvMainCellDblClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
  btnOkClick(btnOk);
...
procedure TPopupListForm.tvMainKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TPopupListForm.aDelExecute(Sender: TObject);
begin
  DelData;
end;
