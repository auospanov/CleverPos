...
function SetCurSklad(ID : Integer) : Boolean;
  ...
  try
    ...
    with MainDM.spTochka do begin
      ...
      try
        ...
        i := 0;
        while not Eof do begin
          SkladArr[i].ID := FieldByName('g_tochka').AsInteger;
...
begin
  ...
  MainDM.spFillCurParams.ParamByName('G_OFFICIAL_').AsInteger := ID;
...
begin
  ...
  MainDM.spSetSklad.ParamByName('ID_').AsInteger := ID;
...
begin
  ...
  MainDM.spFillSchedule.ParamByName('g_tochka_').AsInteger := CurSklad;
