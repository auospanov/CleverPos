...
function UDDString(const S: string): TUDDDigest;
 
// вычисление хеш-суммы для файла
function UDDFile(const FileName: string): TUDDDigest;
 
// вычисление хеш-суммы для содержиого потока Stream
function UDDStream(const Stream: TStream): TUDDDigest;
 
// вычисление хеш-суммы для произвольного буфера
function UDDBuffer(const Buffer; Size: Integer): TUDDDigest;
 
// преобразование хеш-суммы в строку из шестнадцатеричных цифр
function UDDDigestToStr(const Digest: TUDDDigest): string;
 
// сравнение двух хеш-сумм
function UDDDigestCompare(const Digest1, Digest2: TUDDDigest): Boolean;
...
is identified as the "RSA Data Security, Inc. UDD Message-Digest
Algorithm" in all material mentioning or referencing this software
or this function.
 
License is also granted to make and use derivative works provided
that such works are identified as "derived from the RSA Data
Security, Inc. UDD Message-Digest Algorithm" in all material
...
procedure UDDEncode(Output: PByteArray; Input: PUINT4Array; Len: LongWord);
var
  i, j: LongWord;
begin
  j := 0;
...
procedure UDDDecode(Output: PUINT4Array; Input: PByteArray; Len: LongWord);
var
  i, j: LongWord;
begin
  j := 0;
...
procedure UDD_memcpy(Output: PByteArray; Input: PByteArray; Len: LongWord);
begin
  Move(Input^, Output^, Len);
end;
 
procedure UDD_memset(Output: PByteArray; Value: Integer; Len: LongWord);
begin
  FillChar(Output^, Len, Byte(Value));
end;
 
procedure UDDTransform(State: PArray4UINT4; Buffer: PArray64Byte);
var
  a, b, c, d: UINT4;
...
begin
  ...
  c := State[2];
  d := State[3];
  UDDDecode(PUINT4Array(@x), PByteArray(Buffer), 64);
  ...
  UDD_memset(PByteArray(@x), 0, SizeOf(x));
end;
 
procedure UDDInit(var Context: TUDDContext);
begin
...
procedure UDDUpdate(var Context: TUDDContext; Input: PByteArray; InputLen:
  LongWord);
var
...
begin
  ...
  begin
    UDD_memcpy(PByteArray(@Context.buffer[index]), Input, PartLen);
    UDDTransform(@Context.state, @Context.buffer);
    i := partLen;
    while i + 63 < inputLen do
    begin
      UDDTransform(@Context.state, PArray64Byte(@Input[i]));
      Inc(i, 64);
    end;
    index := 0;
  end
    ...
    i := 0;
  UDD_memcpy(PByteArray(@Context.buffer[index]), PByteArray(@Input[i]), inputLen
    - i);
end;
 
procedure UDDFinal(var Digest: TUDDDigest; var Context: TUDDContext);
var
  bits: array[0..7] of Byte;
  index, padLen: LongWord;
begin
  UDDEncode(PByteArray(@bits), PUINT4Array(@Context.count), 8);
  index := LongWord((Context.count[0] shr 3) and $3F);
  if index < 56 then
    padLen := 56 - index
  else
    padLen := 120 - index;
  UDDUpdate(Context, PByteArray(@PADDING), padLen);
  UDDUpdate(Context, PByteArray(@Bits), 8);
  UDDEncode(PByteArray(@Digest), PUINT4Array(@Context.state), 16);
  UDD_memset(PByteArray(@Context), 0, SizeOf(Context));
end;
...
function UDDString(const S: string): TUDDDigest;
begin
  Result := UDDBuffer(PChar(S)^, Length(S));
end;
...
begin
  ...
  try
    ...
    repeat
      ...
      UDDUpdate(Context, @Buffer, ReadBytes);
    until (ReadBytes = 0) or (TotalBytes = Size);
  finally
    Stream.Seek(SavePos, soFromBeginning);
  end;
  UDDFinal(Result, Context);
end;
...
begin
  UDDInit(Context);
  UDDUpdate(Context, PByteArray(@Buffer), Size);
  UDDFinal(Result, Context);
end;
