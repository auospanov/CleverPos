...
//    function UpdData: Boolean; override;
  public
    ...
    property ParentRecID: Integer read FParentRecID write FParentRecID default 0;
  ...
  if OpenMode in [omView, omUpdate, omInsertLike, omInsertChild] then begin
    try
      ...
      if KeyFieldIsStr then
        ...
        spGetData.ParamByName(KeyField).AsInteger := RecID;
        ...
        spIns.ParamByName(FParentKeyField).Value := Null;
{
    ...
    begin
      if KeyFieldIsStr then
        ...
        RecId := spIns.ParamByName(KeyField).AsInteger;
      Result := True;
    end
    else
      Exit;
