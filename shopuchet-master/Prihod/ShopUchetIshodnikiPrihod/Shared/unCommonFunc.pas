...
uses
  ...
  DateUtils, smsc_api, StrUtils, UITypes;
...
var
  ...
  ShowCostPrice : Boolean;    //показывать закупочные цены в валюте
  OldAddPrihod : Boolean;     //делать приход по старому варианту
  ...
  if DirectoryExists(ExportPath) then begin
    ...
    if Copy(ExportDataType, 3, 1) = '1' then begin
      try
        ...
        SL.ADD('<PRODUCTS>');
        ReportsDM.spGetProductList.ParamByName('g_tochka_').AsInteger := CurSklad;
...
begin
  ...
  MainDM.spInsSmsHistory.ParamByName('g_client_').AsInteger := GClient;
