inherited DocForm: TDocForm
  Left = 396
  Top = 120
  Caption = ''
  ClientHeight = 464
  ClientWidth = 599
  OnCreate = FormCreate
  ExplicitWidth = 605
  ExplicitHeight = 490
  PixelsPerInch = 96
  TextHeight = 14
  inherited Panel1: TPanel
    Width = 579
    Height = 419
    ExplicitWidth = 579
    ExplicitHeight = 419
    object pcMain: TcxPageControl
      Left = 2
      Top = 2
      Width = 575
      Height = 415
      Margins.Left = 2
      Margins.Top = 2
      Margins.Right = 2
      Margins.Bottom = 2
      Align = alClient
      TabOrder = 0
      Properties.ActivePage = tsDocs
      Properties.CustomButtons.Buttons = <>
      Properties.Style = 5
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      ClientRectBottom = 415
      ClientRectRight = 575
      ClientRectTop = 25
      object tsDocs: TcxTabSheet
        Margins.Left = 2
        Margins.Top = 2
        Margins.Right = 2
        Margins.Bottom = 2
        Caption = #1044#1086#1082#1091#1084#1077#1085#1090
        ImageIndex = 0
        ExplicitLeft = -16
        ExplicitTop = 32
        object shp4: TShape
          Left = 227
          Top = 285
          Width = 342
          Height = 48
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object shp3: TShape
          Left = 10
          Top = 285
          Width = 218
          Height = 48
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object shp2: TShape
          Left = 227
          Top = 135
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object shp1: TShape
          Left = 10
          Top = 135
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape9: TShape
          Left = 10
          Top = 85
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape6: TShape
          Left = 227
          Top = 60
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape5: TShape
          Left = 10
          Top = 60
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape44: TShape
          Left = 227
          Top = 160
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape43: TShape
          Left = 10
          Top = 160
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape42: TShape
          Left = 227
          Top = 210
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape41: TShape
          Left = 10
          Top = 210
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape4: TShape
          Left = 227
          Top = 10
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape3: TShape
          Left = 227
          Top = 35
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape26: TShape
          Left = 227
          Top = 235
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape25: TShape
          Left = 10
          Top = 235
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape2: TShape
          Left = 10
          Top = 10
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape16: TShape
          Left = 227
          Top = 260
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape15: TShape
          Left = 10
          Top = 260
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape14: TShape
          Left = 227
          Top = 185
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape13: TShape
          Left = 10
          Top = 185
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape12: TShape
          Left = 227
          Top = 110
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape11: TShape
          Left = 10
          Top = 110
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object Shape10: TShape
          Left = 227
          Top = 85
          Width = 342
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Pen.Color = clBtnShadow
        end
        object Shape1: TShape
          Left = 10
          Top = 35
          Width = 218
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Brush.Color = 16577773
          Pen.Color = clBtnShadow
        end
        object lb2: TLabel
          Left = 18
          Top = 300
          Width = 146
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1054#1089#1085#1086#1074#1072#1085#1080#1077' ('#1087#1088#1080#1084#1077#1095#1072#1085#1080#1077')'
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbClient: TLabel
          Left = 18
          Top = 141
          Width = 41
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1050#1083#1080#1077#1085#1090
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label8: TLabel
          Left = 18
          Top = 266
          Width = 79
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1056#1091#1082#1086#1074#1086#1076#1080#1090#1077#1083#1100
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbClientIIk: TLabel
          Left = 18
          Top = 191
          Width = 141
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1056#1072#1089#1095#1077#1090#1085#1099#1081' '#1089#1095#1077#1090' '#1082#1083#1080#1077#1085#1090#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbCompanyAccount: TLabel
          Left = 18
          Top = 116
          Width = 161
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1056#1072#1089#1095#1077#1090#1085#1099#1081' '#1089#1095#1077#1090' '#1087#1086#1089#1090#1072#1074#1097#1080#1082#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbCompanyAddress: TLabel
          Left = 18
          Top = 91
          Width = 105
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1040#1076#1088#1077#1089' '#1087#1086#1089#1090#1072#1074#1097#1080#1082#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbCompany: TLabel
          Left = 18
          Top = 66
          Width = 137
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1054#1088#1075#1072#1085#1080#1079#1072#1094#1080#1103'-'#1087#1086#1089#1090#1072#1074#1097#1080#1082
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbClientAddress: TLabel
          Left = 18
          Top = 166
          Width = 85
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1040#1076#1088#1077#1089' '#1082#1083#1080#1077#1085#1090#1072
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label21: TLabel
          Left = 18
          Top = 242
          Width = 108
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1043#1083#1072#1074#1085#1099#1081' '#1073#1091#1093#1075#1072#1083#1090#1077#1088
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label2: TLabel
          Left = 18
          Top = 16
          Width = 37
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1053#1086#1084#1077#1088
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object lbOfficial: TLabel
          Left = 18
          Top = 215
          Width = 75
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1048#1089#1087#1086#1083#1085#1080#1090#1077#1083#1100
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label1: TLabel
          Left = 18
          Top = 41
          Width = 71
          Height = 14
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Caption = #1044#1072#1090#1072' '#1074#1099#1076#1072#1095#1080
          Font.Charset = RUSSIAN_CHARSET
          Font.Color = 7492630
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object pnParam2: TPanel
          Left = 7
          Top = 357
          Width = 564
          Height = 33
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 13
          object Shape36: TShape
            Left = 220
            Top = 0
            Width = 342
            Height = 26
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Pen.Color = clBtnShadow
          end
          object Shape35: TShape
            Left = 4
            Top = 0
            Width = 217
            Height = 26
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Brush.Color = 16577773
            Pen.Color = clBtnShadow
          end
          object lbParam2: TLabel
            Left = 11
            Top = 7
            Width = 116
            Height = 14
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Caption = #1050#1088#1077#1076#1080#1090' ('#1082#1086#1088#1088'. '#1089#1095#1077#1090')'
            Font.Charset = RUSSIAN_CHARSET
            Font.Color = 7492630
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            Transparent = True
          end
          object edParam2: TcxTextEdit
            Left = 226
            Top = 3
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Properties.MaxLength = 250
            Style.BorderStyle = ebsFlat
            Style.StyleController = dxEditStyleController
            TabOrder = 0
            Width = 331
          end
        end
        object pnParam1: TPanel
          Left = 3
          Top = 332
          Width = 571
          Height = 26
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 12
          object Shape38: TShape
            Left = 224
            Top = 0
            Width = 342
            Height = 26
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Pen.Color = clBtnShadow
          end
          object Shape37: TShape
            Left = 7
            Top = 0
            Width = 218
            Height = 26
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Brush.Color = 16577773
            Pen.Color = clBtnShadow
          end
          object lbParam1: TLabel
            Left = 15
            Top = 5
            Width = 35
            Height = 14
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Caption = #1044#1077#1073#1077#1090
            Font.Charset = RUSSIAN_CHARSET
            Font.Color = 7492630
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            Transparent = True
          end
          object edParam1: TcxTextEdit
            Left = 230
            Top = 2
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Properties.MaxLength = 250
            Style.BorderStyle = ebsFlat
            Style.StyleController = dxEditStyleController
            TabOrder = 0
            Width = 331
          end
        end
        object edDate: TcxDateEdit
          Left = 233
          Top = 37
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.Alignment.Horz = taLeftJustify
          Properties.InputKind = ikMask
          Properties.OnChange = edDatePropertiesChange
          Style.BorderStyle = ebsFlat
          Style.ButtonStyle = bts3D
          TabOrder = 1
          Width = 331
        end
        object cbbFirm: TcxLookupComboBox
          Left = 233
          Top = 62
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_FIRM_REKVIZIT'
          Properties.ListColumns = <
            item
              FieldName = 'NAME'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsFirm
          Properties.OnChange = cbbFirmPropertiesChange
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 2
          Width = 331
        end
        object cbbClient: TcxLookupComboBox
          Left = 233
          Top = 138
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_CLIENT'
          Properties.ListColumns = <
            item
              FieldName = 'NAME'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsClient
          Properties.OnChange = cbbClientPropertiesChange
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 5
          Width = 331
        end
        object edNum: TcxTextEdit
          Left = 233
          Top = 12
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.MaxLength = 50
          Style.BorderStyle = ebsFlat
          Style.StyleController = dxEditStyleController
          TabOrder = 0
          OnKeyPress = edNumKeyPress
          Width = 331
        end
        object leIspolnitel: TcxLookupComboBox
          Left = 233
          Top = 212
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.DropDownListStyle = lsFixedList
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_OFFICIAL'
          Properties.ListColumns = <
            item
              FieldName = 'FIO'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsIspolnitel
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 8
          Width = 331
        end
        object leGlavBuh: TcxLookupComboBox
          Left = 233
          Top = 237
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_OFFICIAL'
          Properties.ListColumns = <
            item
              FieldName = 'FIO_DOC'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsGlavBuh
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 9
          Width = 331
        end
        object leRukovoditel: TcxLookupComboBox
          Left = 233
          Top = 262
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_OFFICIAL'
          Properties.ListColumns = <
            item
              FieldName = 'FIO_DOC'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsRukovoditel
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 10
          Width = 331
        end
        object mNote: TcxMemo
          Left = 233
          Top = 289
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.MaxLength = 250
          Style.StyleController = dxEditStyleController
          TabOrder = 11
          Height = 41
          Width = 331
        end
        object leMestoClienta: TcxLookupComboBox
          Left = 233
          Top = 163
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_ADDRESS'
          Properties.ListColumns = <
            item
              FieldName = 'ADDRESS'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsMestoClienta
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 6
          Width = 331
        end
        object leIikClient: TcxLookupComboBox
          Left = 233
          Top = 187
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_ACCOUNT'
          Properties.ListColumns = <
            item
              FieldName = 'ACCOUNT'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsIikClient
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 7
          Width = 331
        end
        object leAddress: TcxLookupComboBox
          Left = 233
          Top = 88
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_ADDRESS'
          Properties.ListColumns = <
            item
              FieldName = 'ADDRESS'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsAddress
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 3
          Width = 331
        end
        object leAccount: TcxLookupComboBox
          Left = 233
          Top = 113
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Properties.GridMode = True
          Properties.KeyFieldNames = 'G_ACCOUNT'
          Properties.ListColumns = <
            item
              FieldName = 'ACCOUNT'
            end>
          Properties.ListOptions.ShowHeader = False
          Properties.ListSource = dsAccount
          Style.BorderStyle = ebsFlat
          Style.ButtonTransparency = ebtNone
          TabOrder = 4
          Width = 331
        end
      end
      object tsDetails: TcxTabSheet
        Margins.Left = 2
        Margins.Top = 2
        Margins.Right = 2
        Margins.Bottom = 2
        Caption = #1058#1086#1074#1072#1088#1099
        ImageIndex = 1
        object pn3: TPanel
          Left = 0
          Top = 0
          Width = 575
          Height = 41
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 0
          object btnIns: TBitBtn
            Left = 5
            Top = 8
            Width = 103
            Height = 25
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Caption = #1044#1086#1073#1072#1074#1080#1090#1100
            Glyph.Data = {
              
              B400C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
            NumGlyphs = 2
            TabOrder = 0
            OnClick = btnInsClick
          end
          object btnDel: TBitBtn
            Left = 211
            Top = 8
            Width = 103
            Height = 25
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Caption = #1059#1076#1072#1083#1080#1090#1100
            Glyph.Data = {
              
              B400C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
            NumGlyphs = 2
            TabOrder = 2
            OnClick = btnDelClick
          end
          object btnUpd: TBitBtn
            Left = 108
            Top = 8
            Width = 103
            Height = 25
            Margins.Left = 2
            Margins.Top = 2
            Margins.Right = 2
            Margins.Bottom = 2
            Caption = #1048#1079#1084#1077#1085#1080#1090#1100
            Glyph.Data = {
              
              B400C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
            NumGlyphs = 2
            TabOrder = 1
            OnClick = btnUpdClick
          end
        end
        object gr1: TcxGrid
          Left = 0
          Top = 41
          Width = 575
          Height = 349
          Margins.Left = 2
          Margins.Top = 2
          Margins.Right = 2
          Margins.Bottom = 2
          Align = alClient
          TabOrder = 1
          LookAndFeel.Kind = lfFlat
          object tvTovary: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            DataController.DataSource = dsTovary
            DataController.KeyFieldNames = 'g_product'
            DataController.Summary.DefaultGroupSummaryItems = <>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            OptionsSelection.CellSelect = False
            Styles.Inactive = MainDM.stGridSelectedRow
            object cxgrdbclmnMainRecId: TcxGridDBColumn
              DataBinding.FieldName = 'RecId'
              Visible = False
            end
            object cxgrdbclmnMaing_product: TcxGridDBColumn
              DataBinding.FieldName = 'g_product'
              Visible = False
            end
            object cxgrdbclmnMainname: TcxGridDBColumn
              Caption = #1058#1086#1074#1072#1088
              DataBinding.FieldName = 'name'
              HeaderAlignmentHorz = taCenter
              Width = 275
            end
            object cxgrdbclmnMainamount: TcxGridDBColumn
              Caption = #1050#1086#1083'-'#1074#1086
              DataBinding.FieldName = 'amount'
              HeaderAlignmentHorz = taCenter
              Width = 150
            end
          end
          object lvMain: TcxGridLevel
            GridView = tvTovary
          end
        end
      end
    end
  end
  inherited btnOk: TcxButton
    Left = 407
    Top = 431
    Width = 97
    Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
    ExplicitLeft = 407
    ExplicitTop = 431
    ExplicitWidth = 97
  end
  inherited btnCancel: TcxButton
    Left = 512
    Top = 431
    Caption = #1047#1072#1082#1088#1099#1090#1100
    ExplicitLeft = 512
    ExplicitTop = 431
  end
  inherited btHelp: TcxButton
    Top = 431
    ExplicitTop = 431
  end
  object cxButton1: TcxButton [4]
    Left = 320
    Top = 431
    Width = 75
    Height = 25
    Action = aPrint
    Anchors = [akRight, akBottom]
    LookAndFeel.Kind = lfFlat
    LookAndFeel.NativeStyle = False
    TabOrder = 4
  end
  inherited ApplicationEvents1: TApplicationEvents
    Left = 240
    Top = 480
  end
  inherited dxEditStyleController: TcxEditStyleController
    Left = 168
    Top = 544
    PixelsPerInch = 96
  end
  inherited ActionList: TActionList
    Images = MainDM.imMain
    Left = 88
    Top = 704
    object aPrint: TAction
      Caption = #1055#1077#1095#1072#1090#1100
      ImageIndex = 146
      OnExecute = aPrintExecute
    end
  end
  inherited tranRead: TpFIBTransaction
    Top = 432
  end
  inherited spGetData: TpFIBStoredProc
    SQL.Strings = (
      'EXECUTE PROCEDURE DOCS_GET (?DOCS_, ?ZAKAZ_, ?DOC_TYPE_)')
    StoredProcName = 'DOCS_GET'
    Left = 104
    Top = 384
  end
  inherited tranWrite: TpFIBTransaction
    Left = 152
    Top = 432
  end
  inherited spIns: TpFIBStoredProc
    SQL.Strings = (
      
        'EXECUTE PROCEDURE DOCS_INS (?NUM_, ?DOCS_DATE_, ?G_FIRM_, ?G_FIR' +
        'M_ADDRESS_, ?G_FIRM_ACCOUNT_, ?G_CLIENT_, ?G_CLIENT_ADDRESS_, ?G' +
        '_CLIENT_ACCOUNT_, ?G_OFFICIAL_RUKOVOD_, ?GLAVBUH_, ?G_OFFICIAL_,' +
        ' ?NOTE_, ?PARAM1_, ?PARAM2_, ?PARAM3_, ?PARAM4_, ?PARAM5_, ?PARA' +
        'M6_, ?DOC_TYPE_, ?ZAKAZ_)')
    StoredProcName = 'DOCS_INS'
    Left = 136
    Top = 384
  end
  inherited spUpd: TpFIBStoredProc
    SQL.Strings = (
      
        'EXECUTE PROCEDURE DOCS_UPD (?DOCS_, ?NUM_, ?DOCS_DATE_, ?G_FIRM_' +
        ', ?G_FIRM_ADDRESS_, ?G_FIRM_ACCOUNT_, ?G_CLIENT_, ?G_CLIENT_ADDR' +
        'ESS_, ?G_CLIENT_ACCOUNT_, ?G_OFFICIAL_, ?G_OFFICIAL_RUKOVOD_, ?G' +
        'LAVBUH_, ?NOTE_, ?PARAM1_, ?PARAM2_, ?PARAM3_, ?PARAM4_, ?PARAM5' +
        '_, ?PARAM6_)')
    StoredProcName = 'DOCS_UPD'
    Left = 168
    Top = 384
  end
  object dsAddress: TDataSource
    DataSet = spAddress
    Left = 448
    Top = 80
  end
  object dsAccount: TDataSource
    DataSet = spAccount
    Left = 448
    Top = 112
  end
  object dsIspolnitel: TDataSource
    DataSet = spIspolnitel
    Left = 360
    Top = 239
  end
  object dsIikClient: TDataSource
    DataSet = spIikClient
    Left = 448
    Top = 215
  end
  object dsRukovoditel: TDataSource
    DataSet = spRukovoditel
    Left = 344
    Top = 287
  end
  object dsGlavBuh: TDataSource
    DataSet = spGlavBuh
    Left = 480
    Top = 263
  end
  object dsMestoClienta: TDataSource
    DataSet = spMestoClienta
    Left = 520
    Top = 184
  end
  object spAddress: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM g_read_g_firm_address_list(?G_FIRM_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 416
    Top = 80
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spAccount: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM g_read_g_firm_account_list(?G_FIRM_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 416
    Top = 112
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spMestoClienta: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_ADDRESS_LIST(?G_CLIENT_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 488
    Top = 184
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spIikClient: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_CLIENT_BANK(?G_CLIENT_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 416
    Top = 216
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spRukovoditel: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_RUKOVODITEL_LIST(:GLAVBUH_OR_RUKOVODITEL_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 312
    Top = 288
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spIspolnitel: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_OFFICIAL_LIST')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 328
    Top = 240
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spGlavBuh: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_RUKOVODITEL_LIST(:GLAVBUH_OR_RUKOVODITEL_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 448
    Top = 264
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spFirm: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_FIRM_REKVIZIT')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 320
    Top = 56
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object dsFirm: TDataSource
    DataSet = spFirm
    Left = 352
    Top = 56
  end
  object spClient: TpFIBDataSet
    SelectSQL.Strings = (
      
        'SELECT * FROM G_CLIENT WHERE IS_SUPPLIER = :IS_SUPPLIER_ AND IS_' +
        'CATEGORY = 0')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 320
    Top = 160
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object dsClient: TDataSource
    DataSet = spClient
    Left = 352
    Top = 160
  end
  object dsTovary: TDataSource
    DataSet = mdTovary
    Left = 552
    Top = 32
  end
  object mdTovary: TdxMemData
    Indexes = <>
    SortOptions = []
    Left = 520
    Top = 40
    object strngfldTovaryg_product: TStringField
      FieldName = 'g_product'
    end
    object strngfldMemData1name: TStringField
      DisplayWidth = 250
      FieldName = 'name'
      Size = 250
    end
    object strngfldMemData1amount: TStringField
      DisplayWidth = 20
      FieldName = 'amount'
    end
  end
  object xmlTovary: TXMLDocument
    Left = 488
    Top = 40
    DOMVendorDesc = 'MSXML'
  end
  object spGetNum: TpFIBDataSet
    SelectSQL.Strings = (
      
        'SELECT CAST(MAX(CAST(NUM AS NUMERIC(15)))+1 AS VARCHAR(20)) as N' +
        'um'
      '      FROM DOCS P'
      '     WHERE P.DOC_TYPE = :DOC_TYPE_'
      
        'AND EXTRACT(YEAR FROM P.DOCS_DATE) = EXTRACT(YEAR FROM CAST(:INI' +
        'T_DATE_ AS DATE))'
      'AND ((SELECT * FROM IS_NUMERIC(P.NUM)) <> -999)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 248
    Top = 56
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
end
