...
uses
  ...
  System.Actions, cxEditRepositoryItems, RegularExpressions, dxSkinBlueprint,
...
type
  ...
  TProdazhaTovaraForm = class(TDBEditForm)
    ...
    sbClear: TcxButton;
    tvProductUNIT_NAME: TcxGridDBColumn;
  ...
  public
    ...
    property TovarInserted: Boolean read FTovarInserted write FTovarInserted;
    property NotMinusOstatok: Integer read FNotMinusOstatok write FNotMinusOstatok;
  protected
...
uses unCommonFunc, frmProdazhaTovaraN, frmSaleMarkCodeInput;
...
begin
  ...
  if (AllowSellAbsGoods = 0) and (FNotMinusOstatok = 0) then
    if seOstatok.Value <= 0 then begin
      MessageDlg('Остаток товара на складе 0 и после операции он станет отрицательным. Продолжение невозможно!', mtWarning, [mbOk], 0);
      Exit;
    end;
  ...
  if (FIs_spisanie = 0) AND (NotifyNol = 1) then     // про цену ругаюсь только при продаже, т.к. при списании цена = 0
    if edPrice.Value = 0 then begin
      if MessageDlg('Цена товара указана 0, Вы уверены, что хотите продолжить?', mtWarning, [mbYes, mbNo], 0) <> mrYes then begin
        if edPrice.Enabled then
          ErrControl := edPrice;
        Exit;
      end;
    end;

  if (spRead.FieldByName('is_marked').AsInteger <> 0) and (FIs_spisanie = 0) then begin
...
begin
  ...
  if Self.ModalResult = mrOk then begin
    IF Success then begin
      ...
      ProdazhaTovaraNForm.spZakazTemp.Locate('ZAKAZ_DETAILS', spIns.ParamByName('ZAKAZ_DETAILS_').AsInt64, []);
...
begin
  ...
  WITH spRead DO BEGIN
    ...
    GetLastPrice.ParamByName('G_PRODUCT_').AsInt64 := spRead.FieldByName('G_PRODUCT').AsLargeInt;
    ...
    // ЕСЛИ ПЕРЕМЕЩЕНИЕ, ТО КАКУЮ ЦЕНУ СТАВИТЬ - ЗАВИСИТ ОТ НАСТРОЕК , 0 - ЗАКУПОЧНУЮ
    IF (FIs_spisanie in [1,4]) or ((FIs_spisanie = 3) and (UseSelfPriceOnTransfer = 0)) THEN BEGIN
    ...
    if (FieldByName('is_marked').AsInteger = 1) and (FIs_spisanie = 0) then begin
...
procedure TProdazhaTovaraForm.seSkidkaPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
...
begin
  if (edPrice.Value > 0) then begin
    if rbSkidkaInProcent.Checked then begin
      summ := MyCurrencyFrameForm.Value * (edPrice.Value * (1 - seSkidka.Value/100));
      seSkidkaSumm.Value := (MyCurrencyFrameForm.Value * edPrice.Value) - (MyCurrencyFrameForm.Value * edPrice.Value * (1 - seSkidka.Value/100));
    end else begin
      summ := MyCurrencyFrameForm.Value * (edPrice.Value - seSkidka.Value);
...
begin
  ...
  if (edBarcode.text<>'') then begin
    with spRead do begin
      s := '(T.NAME CONTAINING ''' + edBarcode.text + ''') OR (ARTICUL CONTAINING ''' + edBarcode.text + ''') OR (BARCODE CONTAINING '''+ edBarcode.text + ''')' +
      ' OR (NOMENCLATUR_NUM CONTAINING '''+ edBarcode.text + ''') OR (BARCODE_DOP CONTAINING '''+ edBarcode.text + ''')';
