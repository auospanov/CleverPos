...
uses
  ...
  System.UITypes, cxNavigator, System.Actions, cxEditRepositoryItems,
...
begin
  ...
  try
    ...
    ProdazhaTovaraNForm.Client := spRead.FieldByname('g_client').AsLargeInt;
...
begin
  ...
  aPrint.Enabled := (spRead.RecordCount > 0) and (spRead.FieldByName('IS_PAYED_ALL').AsInteger = 0);
...
procedure TZakazNaRealListForm.aPrintExecute(Sender: TObject);
begin
  ReportsDM.PrepareReport(60);

  ReportsDM.spReport.ParamByName('ZAKAZ_').AsInt64 := spRead.FieldByName('ZAKAZ_REAL_PARENT').AsLargeInt;
...
begin
  ...
  try
    OplataTovaraRealNForm.Client := spRead.FieldByname('g_client').AsLargeInt;
...
begin
  spZakazDetails.PARAMByName('zakaz_').AsInt64 := spReadOplata.FieldByName('zakaz').AsLargeInt;
...
begin
  ...
  spReadOplata.ParamByName('G_CLIENT_').AsInt64 := spRead.FieldByName('G_CLIENT').AsLargeInt;
...
begin
  ...
  try
    OplataTovaraRealNForm.Client := spRead.FieldByname('g_client').AsLargeInt;
    OplataTovaraRealNForm.RecID := spReadOplata.FieldByName('zakaz').AsLargeInt;
...
begin
  if spReadOplata.FieldByName('is_vozvrat').AsInteger = 0 then begin
    if MessageDlg(Format('Удалить оплату № "%s" ?',
      ...
      Exit;
    spDelOplata.ParamByName('zakaz_').AsInt64 := spReadOplata.FieldByName('zakaz').AsLargeInt;
    spDelOplata.ParamByName('is_spisanie_').AsInteger := 0;
    spDelOplata.ParamByName('UPLOAD_TO_EXT_APP_').AsInteger := Integer(UploadToMobile);
    ...
    if MessageDlg(Format('Удалить возврат № "%s" ?',
      [spReadOplata.FieldByName('z_num').AsString]), mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      Exit;
    spDelVozvrat.ParamByName('sklad_').AsInt64 := spReadOplata.FieldByName('zakaz').AsLargeInt;
    spDelVozvrat.ParamByName('ALLOW_SELL_ABSENT_GOODS_').AsInteger := AllowSellAbsGoods;
    spDelVozvrat.ParamByName('UPLOAD_TO_EXT_APP_').AsInteger := Integer(UploadToMobile);
...
begin
  ...
  tvDetailsNew.StoreToIniFile('Settings_grid.ini', False, [], Self.Name + '-' + tvDetailsNew.name);
end;
...
begin
  ...
  try
    ZakazRealParentForm.RecID := spRead.FieldByName('zakaz_real_parent').AsLargeInt;
...
begin
  ...
  spDelNakl.ParamByName('ZAKAZ_REAL_PARENT_').AsInt64 := spRead.FieldByName('ZAKAZ_REAL_PARENT').AsLargeInt;
  spDelNakl.ParamByName('UPLOAD_TO_EXT_APP_').AsInteger := Integer(UploadToMobile);
