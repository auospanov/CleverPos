...
begin
  ...
  with spGetData do begin
    IF not (VarIsNull(ParamByName('DOCS_ID_').AsInt64)) THEN    // ПРИ GETDATA ДОКУМЕНТ НЕ ВСЕГДА БЫВАЕТ СОЗДАН
      RecID :=  ParamByName('DOCS_ID_').AsInt64;
    ...
    cbbFirm.EditValue := ParamByName('G_FIRM_').AsInt64;
    leAddress.EditValue := ParamByName('G_FIRM_ADDRESS_').AsInt64;
    leAccount.EditValue := ParamByName('G_FIRM_ACCOUNT_').AsInt64;
    cbbClient.EditValue := ParamByName('G_CLIENT_').AsInt64;
    leMestoClienta.EditValue := ParamByName('G_CLIENT_ADDRESS_').AsInt64;
    leIikClient.EditValue := ParamByName('G_CLIENT_ACCOUNT_').AsInt64;
    leRukovoditel.EditValue := ParamByName('G_OFFICIAL_RUKOVOD_').AsInt64;
    ...
    if (FDocType = 2) and (RecID > 0) then begin
      try
        ...
        for i := 0 to XmlDocNode.ChildNodes.Count - 1 do begin
          ...
          mdTovary.Insert;
          mdTovary.FieldByName('g_product').AsLargeInt := XmlNode.ChildNodes[0].NodeValue;
...
begin
  with SP do begin
    ...
    ParamByName('G_FIRM_').AsInt64 := cbbFirm.EditValue;
    if leAddress.EditValue > 0 then
      ParamByName('G_FIRM_ADDRESS_').AsInt64 := leAddress.EditValue;
    if leAccount.EditValue > 0 then
      ParamByName('G_FIRM_ACCOUNT_').AsInt64 := leAccount.EditValue;
    ParamByName('G_CLIENT_').AsInt64 := cbbClient.EditValue;
    if leMestoClienta.EditValue > 0 then
      ParamByName('G_CLIENT_ADDRESS_').AsInt64 := leMestoClienta.EditValue;
    if leIikClient.EditValue > 0 then
      ParamByName('G_CLIENT_ACCOUNT_').AsInt64 := leIikClient.EditValue;
    if leRukovoditel.EditValue > 0 then
      ParamByName('G_OFFICIAL_RUKOVOD_').AsInt64 := leRukovoditel.EditValue;
...
begin
  ...
  ReportsDM.spReport.ParamByName('Doc_').AsInt64 := RecId;
  ReportsDM.spReport.ParamByName('Zakaz_').AsInt64 := FZakaz;
...
begin
  ...
  spRukovoditel.First;
  leRukovoditel.EditValue := spRukovoditel.FieldByName('g_official').AsLargeInt;
...
procedure TDocForm.edDatePropertiesChange(Sender: TObject);
var num : string;
begin
  spGetNum.ParamByName('INIT_DATE_').AsDate := edDate.Date;
  spGetNum.ParamByName('DOC_TYPE_').AsInteger := FDocType;
...
begin
  spIikClient.ParamByName('g_client_').AsInt64 := cbbClient.editValue;
  OpenSP(spIikClient, False);
  spIikClient.Locate('IS_DEFAULT', 1, []);
  leIikClient.EditValue := spIikClient.FieldByName('g_account').AsLargeInt;
  spMestoClienta.ParamByName('g_client_').AsInt64 := cbbClient.editValue;
  OpenSP(spMestoClienta, False);
  spMestoClienta.Locate('IS_DEFAULT', 1, []);
  leMestoClienta.EditValue := spMestoClienta.FieldByName('g_address').AsLargeInt;
end;

procedure TDocForm.cbbFirmPropertiesChange(Sender: TObject);
begin
  spAddress.ParamByName('g_firm_').AsInt64 := cbbFirm.editValue;
  OpenSP(spAddress, False);
  spAddress.Locate('IS_DEFAULT', 1, []);
  leAddress.EditValue := spAddress.FieldByName('g_address').AsLargeInt;
  spAccount.ParamByName('g_firm_').AsInt64 := cbbFirm.editValue;
  ...
  with PriceListUnSostavForm do begin
    try
      ...
      if ShowModal = mrOk then begin
        ...
        if mdTovary.Locate('g_product', spRead.FieldByName('g_product').AsLargeInt, []) then begin
          mdTovary.Edit;
          mdTovary.FieldByName('amount').AsFloat := mdTovary.FieldByName('amount').AsFloat + StrToFloat(MyCurrencyFrameForm.edAmount.Text);
        end else begin
          mdTovary.Insert;
          mdTovary.FieldByName('g_product').AsLargeInt := spRead.FieldByName('g_product').AsLargeInt;
        ...
        mdTovary.Refresh;
        mdTovary.Locate('g_product', spRead.FieldByName('g_product').AsLargeInt, []);
      end;
    finally
      Free;
    end;
...
begin
  ...
  with PriceListUnSostavForm do begin
    try
      edPrice.Enabled := False;
      edPriceVol.Enabled := False;
      Id := mdTovary.FieldByName('g_product').AsLargeInt;
      Typ := 3;       // чтобы отражались все продукты
