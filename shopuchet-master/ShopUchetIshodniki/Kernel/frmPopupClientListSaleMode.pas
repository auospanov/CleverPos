unit frmPopupClientListSaleMode;
...
type
  TPopupClientListSaleModeForm = class(TPopupListForm)
    Label10: TLabel;
    ...
    clmMainCARD_NUMBER: TcxGridDBColumn;
...
var
  PopupClientListSaleModeForm: TPopupClientListSaleModeForm;

implementation

{$R *.dfm}

procedure TPopupClientListSaleModeForm.CloseClick;
begin
  btnOkClick(btnOK);
end;

procedure TPopupClientListSaleModeForm.edBarcodeFocusChanged(Sender: TObject);
begin
  inherited;
...
procedure TPopupClientListSaleModeForm.edBarcodeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then
    spRead.Prior
...
procedure TPopupClientListSaleModeForm.edBarcodePropertiesChange(Sender: TObject);
begin
...
procedure TPopupClientListSaleModeForm.FormCreate(Sender: TObject);
begin
  inherited;
  KeyFieldIsStr := False;
  KeyFieldList  := 'ID';
...
procedure TPopupClientListSaleModeForm.ReadList;
begin
  inherited;
...
procedure TPopupClientListSaleModeForm.TimerChangeEdTimer(Sender: TObject);
var s : string;
begin
  TimerChangeEd.Enabled := False;
  if (edBarcode.text<>'') then begin
    with spRead do begin
      ...
      ''') OR (NOTE CONTAINING ''' + edBarcode.text +
...
procedure TPopupClientListSaleModeForm.sbClearClick(Sender: TObject);
begin
  edBarcode.Clear;
end;
