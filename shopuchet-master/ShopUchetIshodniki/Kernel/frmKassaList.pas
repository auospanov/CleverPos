...
uses
  ...
  dxSkinTheAsphaltWorld, dxSkinVS2010, dxSkinWhiteprint, unLog;
...
type
  TKassaListForm = class(TDBListForm)
    ...
    tvMainZAKAZ: TcxGridDBColumn;
    tvMainSKLAD: TcxGridDBColumn;
...
begin
  ...
  spRead.ParamByName('action_').AsInt64 := FAction;
  spRead.ParamByName('g_payment_type_').AsInt64 := bcNalBeznal.ItemIndex;

  inherited;

  if (spRead.FieldByName('sklad').AsLargeInt = 0) and (spRead.FieldByName('zakaz').AsLargeInt = 0) then begin
...
begin
  ...
  spDel.ParamByName('g_payment_type_').AsInt64 := spRead.FieldByName('g_payment_type').AsLargeInt;
...
function TKassaListForm.FillLog: string;
begin
  FNewRec.StatyaName := spRead.FieldByname('STATYA_NAME').AsString;
  FNewRec.Kdate := spRead.FieldByname('ACTION_DATE').AsDateTime;
  FNewRec.summa := spRead.FieldByname('SUMM').AsFloat;
  FNewRec.PayTypeName := spRead.FieldByname('PAYMENT_TYPE').AsString;
  Result := FillLogKassa(1, FAction, FNewRec, FNewRec);
end;
