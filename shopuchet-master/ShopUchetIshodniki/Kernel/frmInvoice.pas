...
procedure TInvoiceForm.edDatePropertiesChange(Sender: TObject);
var num : string;
begin
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      FGClient          := ParamByName('G_CLIENT_').AsInt64;
      edClient.Text := ParamByName('CLIENT_NAME_').AsString;
      edIDNClient.Text := ParamByName('IDN_CLIENT_').AsString;

      spMestoClienta.ParamByName('g_client_').AsInt64 := FGClient;
      OpenSP(spMestoClienta, False);
      spMestoClienta.First;
      leMestoClienta.EditValue := ParamByName('ADDRESS_POKUPATEL_').AsInt64;
      ...
      spIikClient.ParamByName('G_CLIENT_').AsInt64 := FGClient;
      OpenSP(spIikClient, False);
      leIikClient.EditValue := ParamByName('IIK_CLIENT_').AsInt64;
      ...
      ELSE
        leGlavBuh.EditValue := ParamByName('G_OFFICIAL_GB_').AsInt64;
  ...
  with SP do begin
    ...
    if not VarIsNull(leMestoClienta.EditValue) then
      if leMestoClienta.EditValue <> 0 then
        ParamByName('ADDRESS_POKUPATEL_').AsInt64 := leMestoClienta.EditValue;
    if not VarIsNull(leIikClient.EditValue) then
      if leIikClient.EditValue <> 0 then
        ParamByName('IIK_CLIENT_').AsInt64 := leIikClient.EditValue;
    ...
    IF leGlavBuh.EditValue > 0 THEN
      ParamByName('G_OFFICIAL_GB_').AsInt64 := leGlavBuh.EditValue;
...
begin
  ...
  ReportsDM.spReport.ParamByName('INVOICE_').AsInt64 := RecId;
...
begin
  ...
  if Success then begin
    ...
    if ManyFirm = 1 then
      SaveSettingsIni('ManyFirm', 'FirmId', null, leFirm.EditValue);
  end;
...
begin
  ...
  if spIspolnitel.RecordCount = 1 then
    leIspolnitel.EditValue := spIspolnitel.FieldByName('g_official').AsLargeInt;
