object ProdazhaTovaraNForm: TProdazhaTovaraNForm
  Left = 362
  Top = 53
  Hint = #1053#1072#1078#1084#1080#1090#1077', '#1095#1090#1086#1073#1099' '#1086#1087#1083#1072#1095#1080#1074#1072#1077#1084#1072#1103' '#1089#1091#1084#1084#1072' '#1089#1090#1072#1083#1072' '#1088#1072#1074#1085#1072' '#1089#1091#1084#1084#1077' '#1087#1088#1086#1076#1072#1078#1080
  Anchors = [akLeft, akTop, akBottom]
  BorderIcons = [biSystemMenu, biMaximize]
  BorderStyle = bsSingle
  Caption = #1054#1092#1086#1088#1084#1083#1077#1085#1080#1077' '#1079#1072#1082#1072#1079#1072
  ClientHeight = 614
  ClientWidth = 652
  Color = clBtnHighlight
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -14
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnShow = FormShow
  DesignSize = (
    652
    614)
  PixelsPerInch = 120
  TextHeight = 16
  object btnOk: TBitBtn
    Left = 432
    Top = 577
    Width = 112
    Height = 32
    Hint = #1053#1072#1078#1084#1080#1090#1077', '#1095#1090#1086#1073#1099' '#1089#1086#1093#1088#1072#1085#1080#1090#1100' '#1085#1072#1082#1083#1072#1076#1085#1091#1102
    Anchors = [akRight, akBottom]
    Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
    Default = True
    ModalResult = 1
    TabOrder = 2
    Glyph.Data = {
      
      9700C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    NumGlyphs = 2
  end
  object btnCancel: TBitBtn
    Left = 554
    Top = 577
    Width = 92
    Height = 32
    Hint = #1042#1099#1093#1086#1076' '#1073#1077#1079' '#1089#1086#1093#1088#1072#1085#1077#1085#1080#1103
    Anchors = [akRight, akBottom]
    Cancel = True
    Caption = #1054#1090#1084#1077#1085#1072
    ModalResult = 2
    TabOrder = 3
    Glyph.Data = {
      
      B800C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    NumGlyphs = 2
  end
  object Panel1: TPanel
    Left = 6
    Top = 6
    Width = 640
    Height = 563
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelInner = bvLowered
    ParentColor = True
    TabOrder = 0
    object Panel2: TPanel
      Left = 2
      Top = 196
      Width = 636
      Height = 365
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object pcMain: TcxPageControl
        Left = 0
        Top = 0
        Width = 636
        Height = 365
        ActivePage = tsCommon
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        ClientRectBottom = 365
        ClientRectRight = 636
        ClientRectTop = 27
        object tsCommon: TcxTabSheet
          Caption = #1044#1077#1090#1072#1083#1080
          ImageIndex = 0
          object Panel4: TPanel
            Left = 0
            Top = 0
            Width = 636
            Height = 41
            Align = alTop
            BevelOuter = bvNone
            TabOrder = 0
            DesignSize = (
              636
              41)
            object BitBtn10: TBitBtn
              Left = 263
              Top = 4
              Width = 123
              Height = 30
              Action = aDel
              Caption = #1059#1076#1072#1083#1080#1090#1100
              ParentShowHint = False
              ShowHint = True
              TabOrder = 2
              Glyph.Data = {
                
                B400C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
              NumGlyphs = 2
            end
            object BitBtn12: TBitBtn
              Left = 6
              Top = 4
              Width = 153
              Height = 30
              Action = aIns
              Caption = #1044#1086#1073#1072#1074#1080#1090#1100' '#1090#1086#1074#1072#1088
              ParentShowHint = False
              ShowHint = True
              TabOrder = 0
              Glyph.Data = {
                
                B400C9C9C900FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
              NumGlyphs = 2
            end
            object BitBtn9: TBitBtn
              Left = 158
              Top = 4
              Width = 107
              Height = 30
              Action = aUpd
              Caption = #1048#1079#1084#1077#1085#1080#1090#1100
              ParentShowHint = False
              ShowHint = True
              TabOrder = 1
              Glyph.Data = {
                
                FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
              NumGlyphs = 2
            end
            object pnRowCnt: TPanel
              Left = 685
              Top = 4
              Width = 63
              Height = 30
              Anchors = [akTop, akRight]
              BevelInner = bvRaised
              BevelOuter = bvLowered
              Caption = '0'
              TabOrder = 3
            end
          end
          object grProduct: TdxDBGrid
            Left = 0
            Top = 41
            Width = 636
            Height = 161
            BandRowCount = 2
            Bands = <
              item
              end>
            DefaultLayout = False
            HeaderPanelRowCount = 1
            KeyField = 'ZAKAZ_DETAILS'
            ShowSummaryFooter = True
            SummaryGroups = <>
            SummarySeparator = ', '
            Align = alClient
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            OnDblClick = grProductDblClick
            BandFont.Charset = DEFAULT_CHARSET
            BandFont.Color = clWindowText
            BandFont.Height = -11
            BandFont.Name = 'MS Sans Serif'
            BandFont.Style = []
            DataSource = dsZakazTemp
            Filter.Criteria = {00000000}
            HeaderFont.Charset = DEFAULT_CHARSET
            HeaderFont.Color = clWindowText
            HeaderFont.Height = -13
            HeaderFont.Name = 'MS Sans Serif'
            HeaderFont.Style = []
            OptionsBehavior = [edgoAutoSearch, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoMouseScroll, edgoMultiSelect, edgoMultiSort, edgoStoreToRegistry, edgoTabThrough, edgoVertThrough]
            OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
            PreviewFont.Charset = DEFAULT_CHARSET
            PreviewFont.Color = clBlue
            PreviewFont.Height = -11
            PreviewFont.Name = 'MS Sans Serif'
            PreviewFont.Style = []
            RegistryPath = '\Software\ShopUchet\ProdazhaTovaraNakl'
            object grProductPRODUCT_NAME: TdxDBGridMaskColumn
              Alignment = taLeftJustify
              Caption = #1058#1086#1074#1072#1088
              HeaderAlignment = taCenter
              Width = 80
              BandIndex = 0
              RowIndex = 0
              FieldName = 'PRODUCT_NAME'
            end
            object grProductSKLAD: TdxDBGridMaskColumn
              Alignment = taCenter
              HeaderAlignment = taCenter
              Visible = False
              BandIndex = 0
              RowIndex = 0
              FieldName = 'SKLAD'
            end
            object grProductG_PRODUCT: TdxDBGridMaskColumn
              Alignment = taCenter
              HeaderAlignment = taCenter
              Visible = False
              BandIndex = 0
              RowIndex = 0
              FieldName = 'G_PRODUCT'
            end
            object grProductAMOUNT: TdxDBGridMaskColumn
              Alignment = taRightJustify
              Caption = #1050#1086#1083'-'#1074#1086
              HeaderAlignment = taCenter
              Width = 104
              BandIndex = 0
              RowIndex = 0
              FieldName = 'AMOUNT'
              SummaryFooterType = cstSum
            end
            object grProductPRICE: TdxDBGridMaskColumn
              Alignment = taRightJustify
              Caption = #1062#1077#1085#1072
              HeaderAlignment = taCenter
              Width = 60
              BandIndex = 0
              RowIndex = 0
              FieldName = 'PRICE'
            end
            object grProductDISCOUNT_PERC: TdxDBGridColumn
              Alignment = taRightJustify
              Caption = #1057#1082#1080#1076#1082#1072' %'
              HeaderAlignment = taCenter
              BandIndex = 0
              RowIndex = 0
              FieldName = 'DISCOUNT_PERC'
            end
            object grProductSUMM: TdxDBGridMaskColumn
              Alignment = taCenter
              Caption = #1057#1091#1084#1084#1072
              HeaderAlignment = taCenter
              Width = 60
              BandIndex = 0
              RowIndex = 0
              FieldName = 'SUMM'
              SummaryFooterType = cstSum
            end
            object grProductNDS: TdxDBGridMaskColumn
              Alignment = taRightJustify
              Caption = #1053#1044#1057'%'
              HeaderAlignment = taCenter
              Width = 60
              BandIndex = 0
              RowIndex = 0
              FieldName = 'NDS'
            end
            object grProductNDS_SUMM: TdxDBGridMaskColumn
              Alignment = taRightJustify
              Caption = #1057#1091#1084#1084#1072' '#1053#1044#1057
              HeaderAlignment = taCenter
              Width = 97
              BandIndex = 0
              RowIndex = 0
              FieldName = 'NDS_SUMM'
              SummaryFooterType = cstSum
            end
            object grProductDISCOUNT_SUMM: TdxDBGridColumn
              Alignment = taRightJustify
              Caption = #1057#1082#1080#1076#1082#1072' ('#1089#1091#1084#1084')'
              HeaderAlignment = taCenter
              BandIndex = 0
              RowIndex = 0
              FieldName = 'DISCOUNT_SUMM'
            end
          end
          object Panel5: TPanel
            Left = 0
            Top = 202
            Width = 636
            Height = 136
            Align = alBottom
            BevelOuter = bvNone
            ParentColor = True
            TabOrder = 2
            DesignSize = (
              636
              136)
            object pnlOplata: TPanel
              Left = 4
              Top = 41
              Width = 344
              Height = 124
              Anchors = [akLeft, akTop, akRight]
              BevelOuter = bvNone
              ParentColor = True
              TabOrder = 2
              DesignSize = (
                344
                124)
              object Shape5: TShape
                Left = 5
                Top = 31
                Width = 101
                Height = 32
                Brush.Color = 16577773
                Pen.Color = clBtnShadow
              end
              object Shape3: TShape
                Left = 5
                Top = 0
                Width = 101
                Height = 32
                Brush.Color = 16577773
                Pen.Color = clBtnShadow
              end
              object Label3: TLabel
                Left = 15
                Top = 6
                Width = 67
                Height = 18
                Caption = #1054#1087#1083#1072#1095#1077#1085#1086
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = 7492630
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object Label5: TLabel
                Left = 15
                Top = 38
                Width = 79
                Height = 18
                Caption = #1057#1076#1072#1095#1072'/'#1044#1086#1083#1075
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = 7492630
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object Shape6: TShape
                Left = 105
                Top = 31
                Width = 163
                Height = 32
                Anchors = [akLeft, akTop, akRight]
                Pen.Color = clBtnShadow
              end
              object Shape4: TShape
                Left = 105
                Top = 0
                Width = 163
                Height = 32
                Anchors = [akLeft, akTop, akRight]
                Pen.Color = clBtnShadow
              end
              object sbEqualSumm: TSpeedButton
                Left = 272
                Top = 2
                Width = 28
                Height = 28
                Anchors = [akTop, akRight]
                Constraints.MaxHeight = 35
                Constraints.MaxWidth = 35
                Constraints.MinHeight = 22
                Constraints.MinWidth = 23
                Glyph.Data = {
                  
                  0000}
                OnClick = sbEqualSummClick
              end
              object edSummDolg: TdxCurrencyEdit
                Left = 111
                Top = 34
                Width = 151
                Enabled = False
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                Anchors = [akLeft, akTop, akRight]
                Alignment = taRightJustify
                DisplayFormat = ',0.00;-,0.00'
                UseThousandSeparator = True
                StoredValues = 1
              end
              object edSummFact: TdxCurrencyEdit
                Left = 111
                Top = 4
                Width = 151
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                Anchors = [akLeft, akTop, akRight]
                Alignment = taRightJustify
                OnChange = edSummFactChange
                DisplayFormat = ',0.00;-,0.00'
                UseThousandSeparator = True
                StoredValues = 1
              end
              object cbByRecipe: TcxCheckBox
                Left = 4
                Top = 66
                Width = 111
                Height = 25
                Hint = #1055#1086#1089#1090#1072#1074#1100#1090#1077' '#1075#1072#1083#1086#1095#1082#1091', '#1077#1089#1083#1080' '#1076#1072#1085#1085#1072#1103' '#1087#1088#1086#1076#1072#1078#1072' '#1080#1076#1077#1090' '#1087#1086' '#1088#1077#1094#1077#1087#1090#1091
                ParentShowHint = False
                Properties.DisplayUnchecked = 'False'
                Properties.OnChange = cb1PropertiesChange
                Properties.Caption = #1055#1086' '#1088#1077#1094#1077#1087#1090#1091
                ShowHint = True
                Style.BorderStyle = ebsFlat
                TabOrder = 2
              end
              object btnOplata: TBitBtn
                Left = 308
                Top = 2
                Width = 29
                Height = 28
                Action = aOplata
                Anchors = [akTop, akRight]
                ParentShowHint = False
                ShowHint = True
                TabOrder = 3
                Glyph.Data = {
                  
                  B2B2B2B2B2BFBFBFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
                NumGlyphs = 2
              end
            end
            object pnlBonus: TPanel
              Left = 348
              Top = 0
              Width = 288
              Height = 136
              Align = alRight
              BevelOuter = bvNone
              ParentColor = True
              TabOrder = 0
              DesignSize = (
                288
                136)
              object Label11: TLabel
                Left = 30
                Top = 34
                Width = 119
                Height = 18
                Caption = #1041#1086#1085#1091#1089' '#1079#1072' '#1087#1086#1082#1091#1087#1082#1091
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clBlack
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object Label12: TLabel
                Left = 30
                Top = 63
                Width = 98
                Height = 18
                Caption = #1041#1086#1085#1091#1089#1086#1074' '#1074#1089#1077#1075#1086
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clBlack
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object lb1: TLabel
                Left = 6
                Top = 92
                Width = 135
                Height = 18
                Caption = #1041#1086#1085#1091#1089#1072#1084#1080' '#1086#1087#1083#1072#1095#1077#1085#1086
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clBlack
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object edBonus: TdxCurrencyEdit
                Left = 154
                Top = 31
                Width = 123
                Enabled = False
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clNavy
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                Anchors = [akLeft, akTop, akRight, akBottom]
                Constraints.MaxWidth = 160
                Alignment = taRightJustify
                DisplayFormat = ',0.00;-,0.00'
                StoredValues = 1
              end
              object edBonusAll: TdxCurrencyEdit
                Left = 154
                Top = 60
                Width = 123
                Enabled = False
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clNavy
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 2
                Anchors = [akLeft, akTop, akRight, akBottom]
                Constraints.MaxWidth = 160
                Alignment = taRightJustify
                DisplayFormat = ',0.00;-,0.00'
                StoredValues = 1
              end
              object edBonusPayed: TdxCurrencyEdit
                Left = 154
                Top = 90
                Width = 123
                Enabled = False
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = clNavy
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 3
                Anchors = [akLeft, akTop, akRight, akBottom]
                Constraints.MaxWidth = 160
                Alignment = taRightJustify
                DisplayFormat = ',0.00;-,0.00'
                StoredValues = 1
              end
              object cbPayBonus: TcxCheckBox
                Left = 9
                Top = 2
                Width = 179
                Height = 25
                Hint = 
                  #1055#1086#1089#1090#1072#1074#1100#1090#1077' '#1075#1072#1083#1086#1095#1082#1091', '#1077#1089#1083#1080' '#1093#1086#1090#1080#1090#1077', '#1095#1090#1086#1073#1099' '#1082#1083#1080#1077#1085#1090' '#1088#1072#1089#1087#1083#1072#1090#1080#1083#1089#1103' '#1085#1072#1082#1086#1087#1083#1077 +
                  #1085#1085#1099#1084#1080' '#1073#1086#1085#1091#1089#1072#1084#1080'.'
                ParentFont = False
                ParentShowHint = False
                Properties.DisplayUnchecked = 'False'
                Properties.OnChange = cbPayBonusClick
                Properties.Caption = #1054#1087#1083#1072#1090#1080#1090#1100' '#1073#1086#1085#1091#1089#1072#1084#1080
                ShowHint = True
                Style.BorderStyle = ebsFlat
                TabOrder = 0
              end
            end
            object Panel3: TPanel
              Left = 4
              Top = 10
              Width = 277
              Height = 32
              Anchors = [akLeft, akTop, akRight]
              BevelOuter = bvNone
              TabOrder = 1
              DesignSize = (
                277
                32)
              object Shape7: TShape
                Left = 5
                Top = 0
                Width = 101
                Height = 32
                Brush.Color = 16577773
                Pen.Color = clBtnShadow
              end
              object Label4: TLabel
                Left = 15
                Top = 7
                Width = 45
                Height = 18
                Caption = #1057#1091#1084#1084#1072
                Font.Charset = RUSSIAN_CHARSET
                Font.Color = 7492630
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                Transparent = True
              end
              object Shape8: TShape
                Left = 105
                Top = 0
                Width = 163
                Height = 32
                Anchors = [akLeft, akTop, akRight]
                Pen.Color = clBtnShadow
              end
              object edSumm: TdxCurrencyEdit
                Left = 111
                Top = 4
                Width = 151
                Enabled = False
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -15
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                Anchors = [akLeft, akTop, akRight]
                Alignment = taRightJustify
                DisplayFormat = ',0.00;-,0.00'
                UseThousandSeparator = True
                StoredValues = 1
              end
            end
          end
        end
        object tsPrihod: TcxTabSheet
          Caption = #1057#1086#1087#1091#1090#1089#1090#1074#1091#1102#1097#1072#1103' '#1089#1090#1072#1090#1100#1103' '#1087#1088#1080#1093#1086#1076#1072
          ImageIndex = 1
          DesignSize = (
            636
            338)
          object lbStatya: TLabel
            Left = 33
            Top = 21
            Width = 540
            Height = 60
            Anchors = [akLeft, akTop, akRight]
            Caption = 
              #1045#1089#1083#1080' '#1076#1086#1087#1086#1083#1085#1080#1090#1077#1083#1100#1085#1086' '#1082' '#1087#1088#1086#1076#1072#1078#1077', '#1042#1099' '#1087#1086#1083#1091#1095#1072#1077#1090#1077' '#1077#1097#1077'  '#1087#1088#1080#1093#1086#1076' '#13#10#1076#1077#1085#1077#1075' '#1074 +
              ' '#1082#1072#1089#1089#1091', '#1090#1086' '#1091#1082#1072#1078#1080#1090#1077' '#1085#1080#1078#1077' '#1089#1090#1072#1090#1100#1102' '#1087#1088#1080#1093#1086#1076#1072' '#1080' '#1089#1091#1084#1084#1091' '#1076#1077#1085#1077#1075'.'#13#10#1053#1072#1087#1088#1080#1084#1077#1088',' +
              ' '#1101#1090#1086' '#1084#1086#1078#1077#1090' '#1073#1099#1090#1100' '#1091#1087#1072#1082#1086#1074#1082#1072' '#1090#1086#1074#1072#1088#1072', '#1080#1083#1080' '#1077#1075#1086' '#1076#1086#1089#1090#1072#1074#1082#1072'.'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -18
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
          end
          object sbClear: TSpeedButton
            Left = 586
            Top = 92
            Width = 32
            Height = 27
            Anchors = [akTop, akRight]
            Glyph.Data = {
              
              FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
            OnClick = sbClearClick
          end
          object leStatya: TdxLookupEdit
            Left = 31
            Top = 92
            Width = 546
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -18
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            Anchors = [akLeft, akTop, akRight]
            ListFieldName = 'NAME'
            KeyFieldName = 'G_PRIHOD_RASHOD'
            ListSource = dsReadStatya
            LookupKeyValue = Null
          end
          object edSummPrihod: TdxCurrencyEdit
            Left = 31
            Top = 129
            Width = 597
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -18
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            Anchors = [akLeft, akTop, akRight]
            Alignment = taRightJustify
            DisplayFormat = ',0.00;-,0.00'
            StoredValues = 1
          end
        end
        object tsReserv: TcxTabSheet
          Caption = #1056#1077#1079#1077#1088#1074' '#1090#1086#1074#1072#1088#1072
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -17
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ImageIndex = 2
          ParentFont = False
          DesignSize = (
            636
            338)
          object lbReserv: TLabel
            Left = 25
            Top = 83
            Width = 573
            Height = 36
            Anchors = [akLeft, akTop, akRight]
            Caption = 
              #1055#1086#1089#1090#1072#1074#1100#1090#1077' '#1075#1072#1083#1086#1095#1082#1091', '#1077#1089#1083#1080' '#1082#1083#1080#1077#1085#1090' '#1087#1083#1072#1085#1080#1088#1091#1077#1090' '#1074#1099#1082#1091#1087#1080#1090#1100' '#1090#1086#1074#1072#1088' '#1087#1086#1079#1078#1077'. '#1055 +
              #1086#1089#1083#1077' '#1087#1086#1083#1085#1086#1081' '#1086#1087#1083#1072#1090#1099' '#1085#1072#1081#1076#1080#1090#1077' '#1076#1072#1085#1085#1091#1102' '#1085#1072#1082#1083#1072#1076#1085#1091#1102' '#1080' '#1089#1085#1080#1084#1080#1090#1077' '#1075#1072#1083#1086#1095#1082#1091'.'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            WordWrap = True
          end
          object cbReserv: TcxCheckBox
            Left = 22
            Top = 17
            Width = 325
            Height = 28
            ParentFont = False
            ParentShowHint = False
            Properties.DisplayUnchecked = 'False'
            Properties.OnChange = cb1PropertiesChange
            Properties.Caption = #1055#1086#1089#1090#1072#1074#1080#1090#1100' '#1090#1086#1074#1072#1088' '#1085#1072' '#1088#1077#1079#1077#1088#1074
            ShowHint = False
            Style.BorderStyle = ebsFlat
            TabOrder = 0
          end
        end
        object tsRecalc: TcxTabSheet
          Caption = #1055#1077#1088#1077#1089#1095#1077#1090' '#1079#1072#1082#1091#1087#1086#1095#1085#1099#1093' '#1094#1077#1085
          ImageIndex = 3
          DesignSize = (
            636
            338)
          object lbRecalc: TLabel
            Left = 26
            Top = 133
            Width = 588
            Height = 72
            Anchors = [akLeft, akTop, akRight]
            Caption = 
              #1045#1089#1083#1080' '#1089#1087#1077#1094#1080#1092#1080#1082#1072' '#1084#1072#1075#1072#1079#1080#1085#1072' '#1090#1072#1082#1086#1074#1072', '#1095#1090#1086' '#1090#1086#1074#1072#1088' '#1074#1089#1077#1075#1076#1072' '#1087#1088#1086#1076#1072#1077#1090#1089#1103' '#1087#1086' '#1087#1088 +
              #1077#1076#1079#1072#1082#1072#1079#1091', '#1090#1086' '#1077#1089#1090#1100' '#1087#1088#1080#1093#1086#1076' '#1090#1086#1074#1072#1088#1072' '#1086#1092#1086#1088#1084#1083#1103#1077#1090#1089#1103' '#1087#1086#1089#1083#1077' '#1087#1088#1086#1076#1072#1078#1080', '#1090#1086' '#1085#1072 +
              #1078#1084#1080#1090#1077' '#1082#1085#1086#1087#1082#1091' '#1074#1099#1096#1077', '#1095#1090#1086#1073#1099' '#1087#1077#1088#1077#1089#1095#1080#1090#1072#1090#1100' '#1089#1077#1073#1077#1089#1090#1086#1080#1084#1086#1089#1090#1100' '#1090#1086#1074#1072#1088#1072' '#1074' '#1076#1072#1085#1085 +
              #1086#1084' '#1079#1072#1082#1072#1079#1077' '#1087#1086' '#1072#1082#1090#1091#1072#1083#1100#1085#1099#1084' '#1079#1072#1082#1091#1087#1086#1095#1085#1099#1084' '#1094#1077#1085#1072#1084'.'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            WordWrap = True
          end
          object btnIns: TBitBtn
            Left = 89
            Top = 36
            Width = 475
            Height = 30
            Action = aRecalcPrices
            Anchors = [akLeft, akTop, akRight]
            Caption = #1055#1077#1088#1077#1089#1095#1080#1090#1072#1090#1100' '#1079#1072#1082#1091#1087#1086#1095#1085#1099#1077' '#1094#1077#1085#1099
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -17
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            NumGlyphs = 2
          end
        end
      end
    end
    object pn1: TPanel
      Left = 2
      Top = 2
      Width = 636
      Height = 194
      Align = alTop
      BevelOuter = bvNone
      ParentColor = True
      TabOrder = 0
      DesignSize = (
        636
        194)
      object shTime: TShape
        Left = 361
        Top = 5
        Width = 94
        Height = 32
        Anchors = [akTop, akRight]
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object ShapeNote: TShape
        Left = 7
        Top = 134
        Width = 170
        Height = 52
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object ShapePayment: TShape
        Left = 7
        Top = 103
        Width = 170
        Height = 32
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object ShapeClient: TShape
        Left = 7
        Top = 66
        Width = 170
        Height = 39
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape12: TShape
        Left = 7
        Top = 5
        Width = 169
        Height = 32
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Shape14: TShape
        Left = 7
        Top = 36
        Width = 169
        Height = 32
        Brush.Color = 16577773
        Pen.Color = clBtnShadow
      end
      object Label7: TLabel
        Left = 20
        Top = 12
        Width = 99
        Height = 18
        Caption = #1044#1072#1090#1072' '#1087#1088#1086#1076#1072#1078#1080
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object LabelDoc: TLabel
        Left = 20
        Top = 43
        Width = 143
        Height = 18
        Caption = #1044#1086#1082#1091#1084#1077#1085#1090'-'#1086#1089#1085#1086#1074#1072#1085#1080#1077
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lbClient: TLabel
        Left = 20
        Top = 78
        Width = 49
        Height = 18
        Caption = #1050#1083#1080#1077#1085#1090
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lbPayment: TLabel
        Left = 20
        Top = 111
        Width = 108
        Height = 18
        Caption = #1057#1087#1086#1089#1086#1073' '#1088#1072#1089#1095#1077#1090#1072
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lbNote: TLabel
        Left = 20
        Top = 151
        Width = 84
        Height = 18
        Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Shape13: TShape
        Left = 175
        Top = 5
        Width = 187
        Height = 32
        Anchors = [akLeft, akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object Shape15: TShape
        Left = 454
        Top = 5
        Width = 176
        Height = 32
        Anchors = [akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object Shape16: TShape
        Left = 175
        Top = 36
        Width = 455
        Height = 32
        Anchors = [akLeft, akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object ShapeClient1: TShape
        Left = 175
        Top = 67
        Width = 455
        Height = 38
        Anchors = [akLeft, akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object ShapePayment1: TShape
        Left = 175
        Top = 104
        Width = 455
        Height = 32
        Anchors = [akLeft, akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object ShapeNote1: TShape
        Left = 175
        Top = 134
        Width = 455
        Height = 52
        Anchors = [akLeft, akTop, akRight]
        Pen.Color = clBtnShadow
      end
      object lbTime: TLabel
        Left = 374
        Top = 12
        Width = 43
        Height = 18
        Alignment = taRightJustify
        Anchors = [akTop, akRight, akBottom]
        Caption = #1042#1088#1077#1084#1103
        Font.Charset = RUSSIAN_CHARSET
        Font.Color = 7492630
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object edDate: TcxDateEdit
        Left = 181
        Top = 8
        Width = 176
        Height = 24
        Anchors = [akLeft, akTop, akRight]
        Properties.InputKind = ikMask
        Properties.OnChange = edDatePropertiesChange
        Style.BorderStyle = ebs3D
        Style.ButtonStyle = bts3D
        TabOrder = 0
      end
      object edTime: TcxTimeEdit
        Left = 463
        Top = 8
        Width = 160
        Height = 24
        Anchors = [akTop, akRight]
        EditValue = 0.000000000000000000
        Properties.BeepOnError = True
        Properties.TimeFormat = tfHourMin
        Style.BorderStyle = ebs3D
        TabOrder = 1
      end
      object edOsnovanie: TdxEdit
        Left = 181
        Top = 40
        Width = 442
        TabOrder = 2
        Anchors = [akLeft, akTop, akRight]
        MaxLength = 150
        StoredValues = 2
      end
      object lePostavshik: TdxLookupEdit
        Left = 181
        Top = 73
        Width = 304
        TabOrder = 4
        Anchors = [akLeft, akTop, akRight]
        OnChange = lePostavshikChange
        ListFieldName = 'NAME'
        KeyFieldName = 'G_CLIENT'
        ListSource = dsPostavshik
        LookupKeyValue = Null
      end
      object btDiscCard: TBitBtn
        Left = 492
        Top = 70
        Width = 133
        Height = 31
        Action = aGetDCard
        Anchors = [akTop, akRight]
        Caption = #1044#1080#1089#1082'. '#1082#1072#1088#1090#1072
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object lePAYMENT_TYPE: TdxLookupEdit
        Left = 181
        Top = 108
        Width = 442
        TabOrder = 5
        Anchors = [akLeft, akTop, akRight]
        OnChange = lePAYMENT_TYPEChange
        ListFieldName = 'NAME'
        KeyFieldName = 'G_PAYMENT_TYPE'
        ListSource = dsPaymentType
        LookupKeyValue = Null
      end
      object mNote: TMemo
        Left = 181
        Top = 140
        Width = 442
        Height = 42
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 6
      end
    end
  end
  object btHelp: TBitBtn
    Left = 10
    Top = 577
    Width = 92
    Height = 32
    Hint = #1042#1099#1079#1086#1074' '#1089#1087#1088#1072#1074#1082#1080
    Anchors = [akLeft, akBottom]
    Caption = '&'#1055#1086#1084#1086#1097#1100
    TabOrder = 1
    OnClick = btHelpClick
    Kind = bkHelp
  end
  object dsPostavshik: TDataSource
    DataSet = spPostavshik
    Left = 335
    Top = 64
  end
  object dsPaymentType: TDataSource
    DataSet = spPaymentType
    Left = 335
    Top = 96
  end
  object dsZakazTemp: TDataSource
    DataSet = spZakazTemp
    Left = 137
    Top = 524
  end
  object ActionList: TActionList
    Images = MainDM.imMain
    Left = 112
    Top = 608
    object aRefreshOfficialRights: TAction
      Caption = 'aRefreshOfficialRights'
      ShortCut = 16466
      Visible = False
    end
    object aShowWhoLockRecord: TAction
      Caption = 'aShowWhoLockRecord'
      Hint = #1055#1086#1082#1072#1079#1072#1090#1100' '#1087#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1103', '#1079#1072#1073#1083#1086#1082#1080#1088#1086#1074#1072#1074#1096#1077#1075#1086' '#1079#1072#1087#1080#1089#1100
      ImageIndex = 1
    end
    object aIns: TAction
      Caption = #1044#1086#1073#1072#1074#1080#1090#1100' '#1090#1086#1074#1072#1088
      OnExecute = aInsExecute
    end
    object aUpd: TAction
      Caption = #1048#1079#1084#1077#1085#1080#1090#1100
      Enabled = False
      OnExecute = aUpdExecute
    end
    object aDel: TAction
      Caption = #1059#1076#1072#1083#1080#1090#1100
      Enabled = False
      OnExecute = aDelExecute
    end
    object aGetDCard: TAction
      Caption = #1044#1080#1089#1082'. '#1082#1072#1088#1090#1072
      ImageIndex = 153
      OnExecute = aGetDCardExecute
    end
    object aBiggerScale: TAction
      Caption = #1059#1074#1077#1083#1080#1095#1080#1090#1100' '#1084#1072#1089#1096#1090#1072#1073' '#1085#1072' 10%'
    end
    object aSmallerScale: TAction
      Caption = #1059#1084#1077#1085#1100#1096#1080#1090#1100' '#1084#1072#1089#1096#1090#1072#1073' '#1085#1072' 10%'
    end
    object aRecalcPrices: TAction
      Caption = #1055#1077#1088#1077#1089#1095#1080#1090#1072#1090#1100' '#1079#1072#1082#1091#1087#1086#1095#1085#1099#1077' '#1094#1077#1085#1099
      OnExecute = aRecalcPricesExecute
    end
    object aOplata: TAction
      Hint = #1057#1087#1080#1089#1086#1082' '#1087#1083#1072#1090#1077#1078#1077#1081
      ImageIndex = 164
      OnExecute = aOplataExecute
    end
  end
  object tranRead: TpFIBTransaction
    DefaultDatabase = MainDM.dbMain
    TimeoutAction = TARollback
    TRParams.Strings = (
      'read'
      'nowait'
      'rec_version'
      'read_committed')
    TPBMode = tpbDefault
    Left = 272
    Top = 64
  end
  object spPostavshik: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_CLIENT_LIST(?G_CLIENT_PARENT_)')
    Filter = 'is_supplier = 0'
    Transaction = tranRead
    Database = MainDM.dbMain
    Filtered = True
    Left = 304
    Top = 64
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spPaymentType: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM G_READ_G_PAYMENT_TYPE_LIST')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 304
    Top = 96
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spZakazTemp: TpFIBDataSet
    SelectSQL.Strings = (
      'SELECT * FROM READ_ZAKAZ_DETAILS_LIST_TMP')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 98
    Top = 513
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object spDel: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      'EXECUTE PROCEDURE DEL_DETAILS_TMP (?ZAKAZ_DETAILS_)')
    StoredProcName = 'DEL_DETAILS_TMP'
    Left = 175
    Top = 524
  end
  object spGetData: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      'EXECUTE PROCEDURE GET_ZAKAZ_DETAILS (?ZAKAZ_, ?IS_SPISANIE_)')
    StoredProcName = 'GET_ZAKAZ_DETAILS'
    Left = 112
    Top = 570
  end
  object spIns: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      
        'EXECUTE PROCEDURE INS_ZAKAZ (?G_TOCHKA_, ?Z_DATE_, ?Z_TIME_, ?G_' +
        'CLIENT_, ?G_PAYMENT_TYPE_, ?OSNOVANIE_, ?IS_SPISANIE_, ?NOTE_, ?' +
        'SUMM_, ?SUMM_FACT_, ?SUMM_BONUS_, ?SUMM_BONUS_GOT_, ?G_PRIHOD_, ' +
        '?SUMM_PRIHOD_, ?BY_RECIPE_, ?IS_RESERVE_, ?PAYMENTS_)')
    StoredProcName = 'INS_ZAKAZ'
    Left = 144
    Top = 570
  end
  object spUpd: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      
        'EXECUTE PROCEDURE UPDATE_ZAKAZ_DETAIL (?ZAKAZ_, ?Z_DATE_, ?Z_TIM' +
        'E_, ?G_CLIENT_, ?G_PAYMENT_TYPE_, ?OSNOVANIE_, ?NOTE_, ?SUMM_, ?' +
        'SUMM_FACT_, ?SUMM_BONUS_, ?SUMM_BONUS_GOT_, ?G_PRIHOD_, ?SUMM_PR' +
        'IHOD_, ?IS_SPISANIE_, ?BY_RECIPE_, ?IS_RESERVE_, ?PAYMENTS_)')
    StoredProcName = 'UPDATE_ZAKAZ_DETAIL'
    Left = 176
    Top = 570
  end
  object tranWrite: TpFIBTransaction
    DefaultDatabase = MainDM.dbMain
    TimeoutAction = TARollback
    TRParams.Strings = (
      'write'
      'nowait'
      'rec_version'
      'read_committed')
    TPBMode = tpbDefault
    Left = 210
    Top = 524
  end
  object spReadStatya: TpFIBDataSet
    SelectSQL.Strings = (
      'select * from G_READ_G_PRIHOD_RASHOD_LIST(?ACTION_)')
    Transaction = tranRead
    Database = MainDM.dbMain
    Left = 244
    Top = 521
    poSQLINT64ToBCD = True
    poAskRecordCount = True
  end
  object dsReadStatya: TDataSource
    DataSet = spReadStatya
    Left = 281
    Top = 521
  end
  object spUpdLock: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      
        'EXECUTE PROCEDURE DO_UPDATE (?ID_, ?TABLE_NAME_, ?ERR_CODE, ?ERR' +
        '_MSG)')
    StoredProcName = 'DO_UPDATE'
    Left = 216
    Top = 570
  end
  object spRecalcPrices: TpFIBStoredProc
    Transaction = tranWrite
    Database = MainDM.dbMain
    SQL.Strings = (
      'EXECUTE PROCEDURE RECALC_SELF_PRICES (?DATE_)')
    StoredProcName = 'RECALC_SELF_PRICES'
    Left = 315
    Top = 527
  end
end
