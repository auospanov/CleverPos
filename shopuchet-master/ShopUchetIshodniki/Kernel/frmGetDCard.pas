...
type
  ...
  public
    ...
    property Discont_card : Int64 read FDiscont_card write FDiscont_card;
  end;
...
begin
  ...
  spGetCard.ParamByName('G_DISCONT_CARD_').AsInt64 := 0;
  SearchCard;
end;
...
procedure TGetDCardForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  inherited;

  if ModalResult = mrOk then
    SaveSettingsIni('Salemode', 'CloseDCardWindow',  Integer(cbCloseDialog.Checked), '');
end;

procedure TGetDCardForm.FormShow(Sender: TObject);
begin
  inherited;
  cbCloseDialog.Checked := ReadIntSettingsIni('Salemode', 'CloseDCardWindow', 0) = 1;

  if Discont_card > 0 then begin
    spGetCard.ParamByName('G_DISCONT_CARD_').AsInt64 := FDiscont_card;
...
begin
  if CallSpWithout(spGetCard) then begin
    with spGetCard do begin
      FClient := ParamByName('g_client_').AsInt64;
      ...
      if FClient = 0 then
        ...
        FSummBonus := Format('%n', [ParamByName('SUMM_').AsFloat]);
