...
uses IdHTTP, IdURI, IdMessage, IdSMTP, IdStrings, IdGlobal, IdGlobalProtocols, StrUtils, SysUtils,
    Forms, Classes, DateUtils, Types, Windows, IdMultipartFormData, Dialogs, IdStack,
...
begin
  ...
  if bal[0] = '-999' then
    MessageDlg('Ошибка. Нет связи с SMSC.RU! Проверьте интернет-соединение!', mtError, [mbOk], 0)
  else if Length(bal) = 1 then
    //_print_debug('Сумма на счете: ' + bal[0])
    Result := bal[0]
  else begin
    errcode := StrToInt(bal[1][2]);
    case errcode of
    ...
    MessageDlg('Ошибка №' + bal[1][2] + ' - ' + ErrMsg, mtError, [mbOk], 0);
    Result := '';
  end;
end;
...
begin
  ...
  _aurl := aurl;
	...
	begin
        ...
        try
          Ini.WritesTRING('SMSSS', 'TEXT', aurl + '?' + arg);
        finally
          Ini.UpdateFile;
          Ini.Free;
        end;}
