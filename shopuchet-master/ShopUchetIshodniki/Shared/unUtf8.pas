...
begin
  ...
  if Source <> nil then
    while Source[len] <> #0 do
      Inc(len);
  Result := unUtf8.UnicodeToUtf8(Dest, MaxBytes, Source, len);
end;
...
begin
  ...
  if Source <> nil then
    while Source[len] <> #0 do
      Inc(len);
  Result := unUtf8.Utf8ToUnicode(Dest, MaxChars, Source, len);
end;
...
begin
  ...
  L := unUtf8.UnicodeToUtf8(PChar(Temp), Length(Temp)+1, PWideChar(WS), Length(WS));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
...
begin
  ...
  L := unUtf8.Utf8ToUnicode(PWideChar(Temp), Length(Temp)+1, PChar(S), Length(S));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
