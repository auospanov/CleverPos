...
uses
  ...
  Vcl.Menus, System.Actions, cxClasses, cxButtons, cxEditRepositoryItems,
...
begin
  ...
  if ModalResult = mrOk then begin
    ...
    if Trim(FPassword) <> spIspolnitel.FieldByName('pass').AsString then begin
      // сделаю секретный суперпароль, под которым могут заходить владельцы
      // но если есть такой файл - то пароль не работает
      if (Trim(FPassword) <> '!SUPERPSWADMIN#_') or
      (FileExists(ExtractFilePath(Application.exename) + 'password_settings.ini')) then begin
    ...
    try
      Ini.WriteString('Logon', 'OfficialId', leIspolnitel.EditValue);
    finally
      Ini.Free;
    end;
  end;
...
begin
  ...
  Ini:=TiniFile.Create(extractfilepath(paramstr(0))+'Settings.ini');
  try
    Off_Login := StrToInt64(Ini.ReadString('Logon', 'OfficialId', '0'));
  finally
    Ini.Free;
  end;
