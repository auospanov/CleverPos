...
function TGridFrameDupl.CheckList(ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := AList.IndexOf(StrToInt64(ARecord.Values[2])) <> - 1;
end;

procedure TGridFrameDupl.ClearList;
begin
  AList.Count := 0;
end;

procedure TGridFrameDupl.clmMainColumn1CustomDrawCell(
...
procedure TGridFrameDupl.CreateList;
begin
  AList := TList<Int64>.Create;
end;

procedure TGridFrameDupl.FreeList;
begin
  AList.Free;
end;

procedure TGridFrameDupl.SetCheckOne(AGridRecord: TcxCustomGridRecord);
begin
  if not CheckList(AGridRecord) then
    AList.Add(StrToInt64(AGridRecord.Values[2]));
end;

procedure TGridFrameDupl.SetRemoveCheckAll(SetCheck : Boolean);
var
...
function TGridFrameDupl.FillArr: String;
var
  i : integer;
begin
...
function TGridFrameDupl.GetCheckedCount: integer;
begin
  result := AList.Count;
end;

procedure TGridFrameDupl.tvMainMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
