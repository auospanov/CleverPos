...
type
  TPopupOfficialListForm = class(TPopupListForm)
    Label10: TLabel;
...
var
  PopupOfficialListForm: TPopupOfficialListForm;

implementation

{$R *.dfm}

procedure TPopupOfficialListForm.CloseClick;
begin
  btnOkClick(btnOK);
end;

procedure TPopupOfficialListForm.edBarcodeFocusChanged(Sender: TObject);
begin
  inherited;
...
procedure TPopupOfficialListForm.edBarcodeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_UP then
    spRead.Prior
...
procedure TPopupOfficialListForm.edBarcodePropertiesChange(Sender: TObject);
begin
...
procedure TPopupOfficialListForm.FormCreate(Sender: TObject);
begin
  inherited;
  KeyFieldIsStr := False;
  KeyFieldList  := 'ID';
...
procedure TPopupOfficialListForm.ReadList;
begin
  inherited;
...
procedure TPopupOfficialListForm.TimerChangeEdTimer(Sender: TObject);
var s : string;
begin
  TimerChangeEd.Enabled := False;
  if (edBarcode.text<>'') then begin
    with spRead do begin
      s := '(NAME CONTAINING ''' + edBarcode.text + ''') OR (KOD CONTAINING ''' + edBarcode.text + ''')';
...
procedure TPopupOfficialListForm.sbClearClick(Sender: TObject);
begin
  edBarcode.Clear;
end;
