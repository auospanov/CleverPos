...
type
  TRashodPrihodForm = class(TDBEditForm)
    ...
    Shape26: TShape;
    edName: TcxTextEdit;
    cbAction: TcxComboBox;
    procedure cbAction1Change(Sender: TObject);
...
procedure TRashodPrihodForm.FormShow(Sender: TObject);
begin
  KeyField := 'g_prihod_rashod_';
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      cbAction.ItemIndex := ParamByName('action_').AsInt64;
...
procedure TRashodPrihodForm.cbAction1Change(Sender: TObject);
begin
  inherited;
