...
uses
  ...
  System.Actions, cxButtons, Inifiles, cxEditRepositoryItems, dxSkinBlueprint,
...
type
  TFirmRekvizitForm = class(TDBEditForm)
    ...
    Shape1: TShape;
    Shape2: TShape;
    cbUseSelfPriceOnTransfer: TcxComboBox;
...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      RecID := spGetData.ParamByName('g_firm_rekvizit__').AsInt64;
  ...
  cbUseSelfPriceOnTransfer.ItemIndex := ReadIntSettingsIni('Common', 'UseSelfPriceOnTransfer', 0);
  UseSelfPriceOnTransfer := cbUseSelfPriceOnTransfer.ItemIndex;
end;
  ...
  if pc1.ActivePage <> tsSklad then begin
    ...
    try
      ...
      if TochkaForm.ShowModal = mrOk then begin
        ...
        SkladArr[High(SkladArr)].ID := spTochka.fieldbyname('g_tochka').AsLargeInt;
  ...
  if pc1.ActivePage <> tsSklad then begin
    if pcClientRekv.ActivePage = tsContact then begin
      ...
      try
        ...
        MyFirmAddressForm.RecID := spReadContact.FieldByName('g_address').AsLargeInt;
        MyFirmAddressForm.GFirmRekvizit := RecID;
      ...
      try
        ...
        MyAddressBankRekForm.RecID := spReadBankRekv.FieldByName('g_account').AsLargeInt;
        MyAddressBankRekForm.GFirmRekvizit := RecID;
    ...
    try
      ...
      TochkaForm.RecID := spTochka.FieldByName('g_tochka').AsLargeInt;
...
begin
  if pc1.ActivePage <> tsSklad then begin
    ...
    if spTochka.FieldByName('g_tochka').AsLargeInt = 1 then begin
      MessageDlg('Данный склад является основным и не может быть удален.', mtConfirmation, [mbOk], 0);
      Exit;
    end;

    if spTochka.FieldByName('g_tochka').AsLargeInt = CurSklad then begin
    ...
    try
      ...
      SP := spDelTochka;
      SP.ParamByName('G_tochka_').AsInt64 := spTochka.FieldByName('G_tochka').AsLargeInt;
      SP.ParamByName('UPLOAD_TO_EXT_APP_').AsInteger := Integer(UploadToMobile);
      ...
      if ExecSPTR(SP) then begin
        ...
        while not spTochka.Eof do begin
          SkladArr[i].ID := spTochka.FieldByName('g_tochka').AsLargeInt;
...
begin
  inherited;
  tvContact.StoreToIniFile('Settings_grid.ini', False, [], Self.Name + '-' + tvContact.name);
...
begin
  ...
  if (ModalResult = mrOk) and (Success = True) then begin
    ...
    if RecID = 0 then
      RecID := spUpd.FieldByName('G_FIRM_REKVIZIT__').AsInt64;

    //Какую цену юзать при перемещении между складами
    UseSelfPriceOnTransfer := cbUseSelfPriceOnTransfer.ItemIndex;
    SaveSettingsIni('Common', 'UseSelfPriceOnTransfer', cbUseSelfPriceOnTransfer.ItemIndex, '');
...
begin
  ...
  if ExecSPTR(spUpd) then begin
    ...
    RefreshFirms;
    RecID := spUpd.ParamByName('G_FIRM_REKVIZIT__').AsInt64;
...
begin
  if MessageDlg('Вы уверены, что хотите удалить компанию "' + spReadFirms.FieldByName('name').AsString + '"?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    spDelFirm.ParamByName('G_FIRM_REKVIZIT_').AsInt64 := spReadFirms.FieldByName('g_firm_rekvizit').AsLargeInt;
...
begin
  RecId := spReadFirms.FieldByName('g_firm_rekvizit').AsLargeInt;
  GetData;
end;
