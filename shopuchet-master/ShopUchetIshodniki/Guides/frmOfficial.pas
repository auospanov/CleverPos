...
begin
  ...
  if OpenMode in [omView, omUpdate, omInsertLike] then begin
    with spGetData do begin
      ...
      leAppointment.EditValue := ParamByName('G_APPOINTMENT_').AsInt64;
      ...
      leSklad.EditValue := ParamByName('g_tochka_').AsInt64;
      cbDoNotSalesComm.Checked := ParamByName('do_not_sales_comm_').AsInteger = 1;
      edBarcode.Text := ParamByName('kod_').AsString;
...
begin
  ...
  with SP do begin
    ...
    ParamByName('G_APPOINTMENT_').AsInt64 := leAppointment.EditValue;
    ...
    ParamByName('do_not_sales_comm_').AsInteger := Integer(cbDoNotSalesComm.Checked);
    ParamByName('kod_').AsString := edBarcode.Text;
...
begin
  ...
  if CurrencyVal = 3 then
    cbIssuer.Properties.Items[1] := 'МВД РУ';
end;
