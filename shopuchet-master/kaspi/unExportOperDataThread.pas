...
uses
  ...
  pFIBStoredProc, pFIBQuery, VCL.Forms, System.StrUtils, unSupport, Inifiles;
...
type
  ...
  protected
    ...
    procedure DoUploadToExternalBonusSystem;
    function  DoLoadFromMobile(token : String) : string;
    procedure DoUploadToTelegram(spRead : TpFIBDataSet);
    function  DoLoadOrdersFromKaspi(token : String) : string;
  public
    ...
    ProcessingNow : boolean;         // флаг, что в данный момент поток выполняется
    constructor Create;
...
uses
  unMobileUploadRoutine, unTelegramUploadRoutine, UnECWID, unKaspiUploadRoutine;
...
var
  ...
  i, j, k: Integer;
  ID_Product, Price_Product: string;
  s: string;
  Amount: Double;
...
begin
  while not Terminated do begin
    if DoNotProcessFirst = False then begin
      if not (DirectoryExists(ExportPath)) and (UploadToMobile = false) and (UploadToTelegram = false) and (ExternalBonusSystemEnable = 0) and (ImportKaspiOrders = false) then begin
      ...
      try
        ...
        if uploadSales or uploadArrivals or UploadToMobile or (ExternalBonusSystemEnable = 1) then begin
          try
                                                                  ...
                                                                  'WHERE AP.IS_LOADING = 1 '+
                                                                    ...
                                                                    'AND EXISTS (SELECT AP1.ID '+
                                                                                 ...
                                                                                 'WHERE AP1.IS_LOADING = 1 '+
                                                                                   'AND AP1.ID = AP.ID '+
                                                                                   'AND AP1.ITEM_TYPE = 0 '+
                                                                                   'AND AP1.OBJECT_TYPE=AP.OBJECT_TYPE ' +
                                                                                 'GROUP BY AP1.ID '+
        ...
        if (Copy(ExportDataType, 7, 1) = '1') or (Copy(ExportDataTypeFtp, 1, 1) = '1') then begin
          ReportsThreadDM.PrepareReport(42);
          ReportsThreadDM.spReport.ParamByName('date_begin_').AsDate := Now;
          ReportsThreadDM.spReport.ParamByName('date_end_').AsDate := Now;
          ReportsThreadDM.spReport.ParamByName('G_TOCHKA_ARR_').AsString := '-' + IntToStr(CurSklad) + '-';
          ReportsThreadDM.ShowReport(ExportPath + 'Продажи '+ CurSkladName + ' по категориям за ' + DateToStr(Date) + '.xls');

          if Copy(ExportDataTypeFtp, 1, 1) = '1' then
            PushToArr(ExportPath + 'Продажи '+ CurSkladName + ' по категориям за ' + DateToStr(Date) + '.xls');
        end;

        if (Copy(ExportDataType, 8, 1) = '1') then begin
          ReportsThreadDM.PrepareReport(112);
          ReportsThreadDM.spReport.ParamByName('date_begin_').AsDate := Now;
          ReportsThreadDM.spReport.ParamByName('date_end_').AsDate := Now;
          ReportsThreadDM.spReport.ParamByName('G_TOCHKA_ARR_').AsString := '-' + IntToStr(CurSklad) + '-';
          ReportsThreadDM.ShowReport(ExportPath + 'Продажи с поставщиком '+ CurSkladName + ' за ' + DateToStr(Date) + '.xls');
        end;
              ...
              try
                try
                  ...
                  spScriptWrite.Script.Text := 'UPDATE SYNC_EXT_APP SET IS_LOADED = 1 WHERE IS_LOADING = 1 ' +
                    'AND IS_LOADED = 0 AND OBJECT_TYPE = 1 AND ITEM_TYPE = 0';
                  resp := ExecScript(spScriptWrite, true);
                  if resp <> '' then begin
            ...
            if spRead.RecordCount > 0 then begin
              ...
              try
                try
                  ...
                  PNT := '05';
                  spScriptWrite.Script.Text := 'UPDATE SYNC_EXT_APP SET IS_LOADED = 1 WHERE IS_LOADING = 1 ' +
                    'AND IS_LOADED = 0 AND OBJECT_TYPE = 2 AND ITEM_TYPE = 0';
                  resp := ExecScript(spScriptWrite, true);
                  if resp <> '' then begin
        ...
        //загрузка заказов с Kaspi магазина
        try
          if (ImportKaspiOrders) and (isProKey)  then begin
            Log('Начало работы получения заказов с Kaspi магазина.');

            DoLoadOrdersFromKaspi(KaspiTokenApiOrders);

            Log('Конец работы получения заказов с Kaspi магазина.');
          end;
        except
          on E: Exception do
          ...
          begin
            ...
            begin
              try
                ...
                case UploadKaspiPricesType of
                  0: Price_Product := 'T.PRICE';
                  1: Price_Product := 'T.VOLUME_PRICE PRICE';
                end;

                spCity.Close;
                spCity.SelectSQL.Text := 'SELECT '+Price_Product+',ks.idcity,ks.adress '+
                  'FROM G_PRODUCT T '+
                  'LEFT OUTER JOIN (SELECT G_PRODUCT, AMOUNT, NAME, L.g_tochka '+
                  'FROM LT_SKLAD_PRODUCT L,G_TOCHKA GT WHERE L.G_TOCHKA = GT.G_TOCHKA) LT ON T.G_PRODUCT = LT.G_PRODUCT '+
                  'INNER JOIN (SELECT c.name as City, t.name as Tochka, p.adress, '+
        ...
        //выгрузка в телеграм приложение
        try
          if (UploadToTelegram) and (isProKey) then
            DoUploadToTelegram(spRead);
        except
          on E: Exception do
            Log('Ошибка работы с Телеграм-ботом. Детали: ' + E.Message);
        end;

        //выгрузка продаж во внешнюю бонусную систему
        try
          if (ExternalBonusSystemEnable = 1) and (isProKey)  then begin
            DoUploadToExternalBonusSystem;
          end;
        except
          on E: Exception do
            Log('Ошибка работы с внешними бонусными системами. Детали: ' + E.Message);
        end;

        // в самом конце удаляю все строки из SYNC_EXT_APP
        try
          if (uploadSales or uploadArrivals) and UploadToMobile then
            spScriptWrite.Script.Text := 'DELETE FROM SYNC_EXT_APP WHERE IS_LOADED = 1 AND IS_LOADED_MOB = 1 AND IS_LOADING = 1 AND ITEM_TYPE = 0;'
          else if (uploadSales or uploadArrivals) then
            spScriptWrite.Script.Text := 'DELETE FROM SYNC_EXT_APP WHERE IS_LOADED = 1 AND IS_LOADING = 1 AND ITEM_TYPE = 0;'
          else
            spScriptWrite.Script.Text := 'DELETE FROM SYNC_EXT_APP WHERE IS_LOADED_MOB = 1 AND IS_LOADING = 1 AND ITEM_TYPE = 0;';
...
var
  ...
  parent_id_old, rec_id, id_to_del : Int64;
...
begin
  ...
  { ===========================================================
   17.10.2023
   теперь логин в мобилку происходит по схеме такой:
   1. В поле логин - указывается логин клиента
   2. В поле password - указывается фраза "guid"
   3. В новое поле guid - указывать новую переменную ExportMobileGuid

   Все это сделано для того, чтобы юзер не парился с паролем, а то вечно
   они его забывают, или меняют и прочее.
   Поэтому сначала проверяю - заполнена ли переменная ExportMobileGuid
   Если нет - то генерирую ее и сохраняю в базу данных
  =========================================================== }
  if ExportMobileGuid = '' then begin
    CreateGUID(MyGUID);
    ExportMobileGuid := GUIDToString(MyGUID);
    try
      spScriptWrite.Script.Text := 'update system_setup set str_value = ''' + ExportMobileGuid +
        ''' where system_setup = 8';
      resp := ExecScript(spScriptWrite, true);
      if resp <> '' then begin
        Log('Ошибка при формировании GUID. PNT: 01.0A ' + resp);
        exit;
      end;
    except
      on E: Exception do begin
        Log('Ошибка при формировании GUID. PNT: 01.0A . Детали: ' + E.Message);
        Exit;
      end;
    end;
  end;
  ...
  // ПОЛУЧАЮ ТОКЕН
  res := GetToken(ExportMobileToken, ExportMobileServer, ExportMobileLogin, ExportMobilePassword, ExportMobileGuid);
  ...
  if Pos('Ошибка', res) > 0 then begin
    if Pos('token is not valid', res) > 0 then begin
      res := GetToken('', ExportMobileServer, ExportMobileLogin, ExportMobilePassword, ExportMobileGuid);
      if Pos('Ошибка', res) > 0 then begin
        Log(res);
        Exit;
      end
  ...
  //=============================================================
  // СТОЛКНУЛИСЬ С ТАКИМ СЛУЧАЕМ, КОГДА У ЮЗЕРА ЗАБЛОКИРОВАЛСЯ АККАУНТ
  // А ЮЗЕРУ БЫЛО ПОФИГ, И В ТЕЧЕНИЕ 3-4 МЕСЯЦЕВ ЗАПИСИ ТУПО КОПИЛИСЬ
  // В ТАБЛИЦЕ. ПОЛУЧАЕТСЯ НАДО ОЧИЩАТЬ ТАБЛИЦУ, БУДУ ЧИСТИТЬ КАЖДЫЕ 30 ДНЕЙ
  // ЕСЛИ ОНИ ЗА 30 ДНЕЙ НЕ ОБРАТИЛИСЬ - ТО НАФИГ ИМ ЗНАЧИТ ВЫГРУЗКА НЕ НУЖНА
  //=============================================================
  pnt := '01.A';
  try
    try
      spRead.SelectSQL.Text := 'SELECT MAX(SYNC_EXT_APP) ID '#13#10+
                               'FROM SYNC_EXT_APP S '#13#10+
                               'WHERE EXISTS (SELECT 1 FROM ZAKAZ Z '#13#10+
                                              'WHERE S.ID = Z.ZAKAZ AND S.OBJECT_TYPE = 1 '#13#10+
                                              'AND Z.Z_DATE < DATEADD(-30 day to current_date)) '#13#10+
                               'OR EXISTS (SELECT 1 FROM SKLAD_PARENT SP WHERE S.ID = SP.SKLAD '#13#10+
                                           'AND S.OBJECT_TYPE = 2 AND '#13#10+
                                           'SP.DATE_RECEIPT < DATEADD(-30 day to current_date))';

      ReadSP(spRead);
      if spRead.RecordCount > 0 then begin
        id_to_del := spRead.FieldByName('ID').AsLargeInt;
        spRead.Close;

        spScriptWrite.Script.Text := 'DELETE FROM SYNC_EXT_APP WHERE SYNC_EXT_APP < ' + id_to_del.ToString();
        resp := ExecScript(spScriptWrite, true);
        if resp <> '' then begin
          Log('Ошибка при удалении старых записей для выгрузки. PNT: 01.A ' + resp);
          exit;
        end;
      end;
    except
      on E: Exception do begin
        Log('Ошибка при удалении старых записей для выгрузки. Детали: ' + E.Message);
        Exit;
      end;
    end;
  finally
  ...
  try
    try
      spRead.SelectSQL.Text := 'SELECT * FROM SYNC_EXT_APP WHERE IS_LOADING = 1 AND IS_LOADED_MOB = 0 AND ITEM_TYPE = 0';
      ReadSP(spRead);
      if spRead.RecordCount = 0 then begin
        spRead.Close;
        ...
        if spRead.RecordCount = 0 then begin
          spRead.Close; //Exit;
          //19.03.2024  Маметов С.А.  Проверим также на наличие записи в таблице KASSA_STATE готовые для отправки в МП
          spRead.SelectSQL.Text := 'SELECT * FROM KASSA_STATE WHERE NEED_TO_EXPORT = 1';
          ReadSP(spRead);
          if spRead.RecordCount = 0 then
            Exit;
        end;
  ...
  try
            ...
            while not Eof do begin
              ...
              try
                if not head_filled then begin
                  ...
                  json_item.AddPair( TJSONPair.Create('date', FieldByName('Z_DATE').AsString  + ' ' + FieldByName('Z_TIME').AsString));
    ...
    begin
      // Log(o.ToString);  //больше не буду сохранять весь JSON, не нужно
      ...
      begin
        ...
        begin
          ...
          if (Pos('refers to not existing row in g_product', errStr) > 0) then begin
            ...
            errStr := GetToken('', ExportMobileServer, ExportMobileLogin, ExportMobilePassword, ExportMobileGuid);
      ...
      try
        ...
        ' UPDATE SYNC_EXT_APP SET IS_LOADED_MOB = 1 WHERE IS_LOADED_MOB = 0 AND IS_LOADING = 1 AND ITEM_TYPE = 0';
...
procedure TExportDataThread.DoUploadToTelegram(spRead : TpFIBDataSet);
var strKassa, strMin, resp: String;
  i, k  : integer;
begin
  Log('Начало выгрузки в Телеграм.');
  if licenseEmail = '' then begin
    Log('Не указана почта для выгрузки в Телеграм. ' + resp);
    Exit;
  end;

  try
    if UploadKassa = 1 then begin
      if spRead.Active then
        spRead.Close;
      spRead.SelectSQL.Text := 'SELECT * FROM P_REP_Z_TELEGRAM';

      resp := ReadSP(spRead);
      if resp <> '' then begin
        Log('Ошибка чтения отчета по кассе. Выгрузка в Телеграм. ' + resp);
        Exit;
      end;
      strKassa  := spRead.FieldByName('STR').AsString;

      resp := SendToTelegram(MainDm.RESTClient, MainDm.RESTRequest,
      MainDm.RESTResponse, licenseEmail, strKassa);
      if resp <> '' then begin
        Log('Ошибка выгрузки кассы в Телеграм. ' + resp);
        Exit;
      end;
    end;

    if UploadMinimumReport = 1 then begin
      if spRead.Active then
        spRead.Close;
      spRead.SelectSQL.Text := 'SELECT * FROM P_REP_ZAYAVKA_POSTAVSHIK(:G_TOCHKA_, :BEGIN_DATE_, :END_DATE_,'+
        ':G_CLIENT_, :DO_SKLAD_, :MODE_, :MODE_VYBOR_, :MODE_VAL_, :NEW_VAL_, :ROUND_MODE_)';
      for i := 0 to High(SkladArr) do begin
        spRead.ParamByName('G_TOCHKA_').AsInt64 := SkladArr[i].ID;
        spRead.ParamByName('BEGIN_DATE_').AsDate := now;
        spRead.ParamByName('END_DATE_').AsDate := now;
        spRead.ParamByName('G_CLIENT_').AsInteger := 0;
        spRead.ParamByName('DO_SKLAD_').AsInteger := 0;
        spRead.ParamByName('MODE_').AsInteger := 0;
        spRead.ParamByName('MODE_VYBOR_').AsInteger := 0;
        spRead.ParamByName('MODE_VAL_').AsInteger := 0;
        spRead.ParamByName('NEW_VAL_').AsInteger := 0;
        spRead.ParamByName('ROUND_MODE_').AsInteger := 0;

        resp := ReadSP(spRead);
        if resp <> '' then begin
          Log('Ошибка чтения минимальных остатков в Телеграм. ' + resp);
          Exit;
        end;

        with spRead do begin
          if RecordCount > 0 then begin
            if strMin = '' then begin
              strMin := strMin + '<b>Товары с остатком меньше минимального.<\/b>\n';
            end;

            strMin := strMin + '===============\n';
            strMin := strMin + '<b>Склад: ' + SkladArr[i].Name + '<\/b>\n';
            strMin := strMin + '===============\n';

            First;
            while not eof do begin
              strMin := strMin + spRead.FieldByName('Name').AsString + ', арт. ' + spRead.FieldByName('ARTICUL').AsString +
                ': ' + spRead.FieldByName('AMOUNT_CUR').AsString + ' шт.\n';
              Next;
            end;
            strMin := strMin + '\n';
          end;
      ...
      resp := SendToTelegram(MainDm.RESTClient, MainDm.RESTRequest,
      MainDm.RESTResponse, licenseEmail, strMin);
      if resp <> '' then begin
        Log('Ошибка выгрузки минимальных остатков в Телеграм. ' + resp);
        Exit;
      end;
    end;
...
procedure TExportDataThread.DoUploadToExternalBonusSystem;
var res, idClient, errIdArr : string;
  bonuses, bonusPercent : Double;
begin
  Log('[внеш. бонусная] - Начало работы с внешними бонусными системами.');
  errIdArr := '';

  // ПРОВЕРЮ ВНАЧАЛЕ ЕСТЬ ЛИ ВООБЩЕ ЗАПИСИ ДЛЯ ВЫГРУЗКИ
  try
    try
      spRead.SelectSQL.Text := 'SELECT * FROM SYNC_EXT_APP WHERE IS_LOADING = 1 AND ITEM_TYPE = 1';
      ReadSP(spRead);
      if spRead.RecordCount = 0 then begin
        Exit;
      end;
      ...
      on E: Exception do begin
        Log('[внеш. бонусная] - Ошибка поиска записей для выгрузки. Детали: ' + E.Message);
        Exit;
      end;
    end;
  finally
  ...
  try
    try
      spRead.SelectSQL.Text := 'SELECT S.SYNC_EXT_APP, '+
                               '      G.PHONE, '+
                               '      Z.SUMM, '+
                               '      Z.SUMM_CASH, '+
                               '      Z.SUMM_NONCASH, '+
                               '      Z.SUMM_BONUS, '+
                               '      COALESCE(GN.NAME, '''') PAYMENT_NAME '+
                              'FROM ZAKAZ Z LEFT OUTER JOIN G_NONCASH_PAY_TYPE GN ON Z.G_NONCASH_PAY_TYPE = GN.G_NONCASH_PAY_TYPE, '+
                              '     G_CLIENT G, SYNC_EXT_APP S '+
                              'WHERE S.IS_LOADING = 1 '+
                              'AND S.ITEM_TYPE = 1 '+
                              'AND S.ID = Z.ZAKAZ '+
                              'AND Z.G_CLIENT = G.G_CLIENT';
      ReadSP(spRead);
      if spRead.RecordCount = 0 then begin
        Exit;
      end;
      ...
      with spRead do begin
        First;
        while not Eof do begin
          res := ExtBonusClass.getUserRekv(true, FieldByName('PHONE').AsString,
            idClient, bonuses, bonusPercent);

          if ContainsText(res, 'Ошибка') then begin
            //Log('[внеш. бонусная] - Ошибка на клиенте ' + FieldByName('PHONE').AsString);  внутри уже все логируется

            if ContainsText(res, 'проверьте соединение с Интернет') then
              Exit;
            errIdArr := errIdArr + FieldByName('PHONE').AsString + ',';

            Continue;
          end;

          res := ExtBonusClass.CreateOrderAndSendMoney(idClient,
              FieldByName('PHONE').AsString,
              FieldByName('SUMM').AsFloat,
              FieldByName('SUMM_CASH').AsFloat,
              FieldByName('SUMM_NONCASH').AsFloat,
              FieldByName('SUMM_BONUS').AsString,FieldByName('PAYMENT_NAME').AsString);

          if ContainsText(res, 'Ошибка') then begin
            //Log('[внеш. бонусная] - Ошибка на клиенте ' + FieldByName('PHONE').AsString);   внутри уже все логируется
            if ContainsText(res, 'проверьте соединение с Интернет') then
              Exit;
            errIdArr := errIdArr + FieldByName('SYNC_EXT_APP').AsString + ',';
          end;
          Next;
        end;
        Close;

        if errIdArr <> '' then begin
          errIdArr := Copy(errIdArr, 1, length(errIdArr)-1);
          errIdArr := ' AND SYNC_EXT_APP NOT IN (' + errIdArr + ')';
        end;

        spScriptWrite.Script.Text := 'DELETE FROM SYNC_EXT_APP WHERE IS_LOADING = 1 '+
          'AND ITEM_TYPE = 1' + errIdArr;
        res := ExecScript(spScriptWrite, true);
        if res <> '' then begin
          Log('[внеш. бонусная] - Ошибка удаления выгруженных данных внеш. бонусной системы. ' + res);
          exit;
        end;
  ...
  Log('[внеш. бонусная] - Конец работы с внешними бонусными системами.');
end;

function TExportDataThread.DoLoadOrdersFromKaspi(token : String) : string;
var resp, err_str, pnt, Url, JsonRes, relatedLink, respOrdersDtl, JsonResOrdersDtl : String;
  JsonResponse, JItemData, JItemOrdersDetails, JItemAttributes, JItemRelationships,
  JItemRelEntries, JItemRelLinks, JsonResponseOrdersDtl, JOrdersDtlAttributes, JOrdersDtlRelationships,
  JItemAttributesOriginAddress, JOrdersDtlAttributesOffer: TJSONObject;
  JResData, JResultOrdersDetails : TJSONArray;
  i, k : integer;
  d : TDateTime;
  summCash, summNonCash, summBonus, summTovar : Double;
  res, paymentsArr, orderId, FOrdersDtlAttributesOfferCode : string;
  pageNumber, pageSize, filterOrdersState, filterOrdersCreationDateGe, filterOrdersCreationDateLe,
  filterOrdersStatus, filterOrdersDeliveryType, filterOrdersSignatureRequired : string;
  DateTimeUTC: TDateTime;

  function IsDataArrayEmpty(const JResponse: string): Boolean;
  var
    JSONObject: TJSONObject;
    DataArray: TJSONArray;
  begin
    Result := False;
    JSONObject := TJSONObject.ParseJSONValue(JResponse) as TJSONObject;
    try
      {Проверяем, существует ли объект "data" и является ли он массивом}
      if JSONObject.TryGetValue<TJSONArray>('data', DataArray) then
      begin
        {Если массив "data" не пуст, то результат True}
        Result := DataArray.Count > 0;
      end
      else
        raise Exception.Create('Ошибка: не найден массив "data" в ответе');
    finally
      JSONObject.Free;
    end;
  end;

  procedure GetSetFromToIni(mode: Integer);
  var
    Ini: TMemIniFile;
  begin
    ...
    try
      if mode = 0 then begin
        DateMilliSecStartForImportKaspi := Ini.ReadString('ExportOperData','DateMilliSecStartForImportKaspi', IntToStr(DateTimeToUnix(Now, False) * 1000));
      end else begin
        Ini.WriteString('ExportOperData','DateMilliSecStartForImportKaspi', IntToStr(DateTimeToUnix(Now, False) * 1000));
        Ini.UpdateFile;
      end;
    finally
      Ini.Free;
...
begin
  Log('Начало загрузки с Kaspi магазина.');
  Result := '';

  TRY
    {Номер страницы с результатом запроса. На одной странице может быть максимум 100 заказов.
    Укажите номер нужной страницы, начиная с 0}
    pageNumber := '0';
    {Количество заказов, которое будет на одной странице в ответе. Максимум — 100}
    pageSize := '100';
    {Состояние заказа:
       NEW - новый
       ...
       ARCHIVE - архивный}
    filterOrdersState := 'NEW';
    {Дата создания заказа. Указывается в миллисекундах
      [$ge] - начальное значение
      [$le] - конечное значение}
    // ToDo FOR TEST Указываем дату и время: 24 сентября 2024 года, 09:00 (UTC)
    // ToDo Далее надо будет вместо DateTimeUTC указать DateMilliSecStartForImportKaspi. А DateTimeUTC удалить вообще
    //DateTimeUTC := EncodeDateTime(2024, 9, 24, 9, 0, 0, 0);
    GetSetFromToIni(0);
    filterOrdersCreationDateGe := DateMilliSecStartForImportKaspi;//IntToStr(DateTimeToUnix(DateTimeUTC, False) * 1000);
    filterOrdersCreationDateLe := IntToStr(DateTimeToUnix(Now, False) * 1000);
    {Статус заказа
      ...
      RETURNED - возвращен}
    filterOrdersStatus := 'APPROVED_BY_BANK';
    {Способ доставки. Используйте, только если filter[orders][state] не равен PICKUP. DELIVERY, KASPI_DELIVERY или PICKUP}
    filterOrdersDeliveryType := 'DELIVERY';//'PICKUP';
    {Подписание документов. Используйте, если filter[orders][state] не равен SIGN_REQUIRED}
    filterOrdersSignatureRequired:= 'true';

    {Склеиваем строку URL с переменными}
    Url := 'https://kaspi.kz/shop/api/v2/orders?' +
         'page[number]='+ pageNumber +
         '&page[size]=' + pageSize +
         '&filter[orders][state]=' + filterOrdersState +
         '&filter[orders][creationDate][$ge]=' + filterOrdersCreationDateGe +
         '&filter[orders][creationDate][$le]=' + filterOrdersCreationDateLe +
         '&filter[orders][status]=' + filterOrdersStatus +
         '&filter[orders][deliveryType]=' + filterOrdersDeliveryType +
         '&filter[orders][signatureRequired]=' + filterOrdersSignatureRequired +
         '&include[orders]=user';

    resp := GetFromKaspi(MainDm.RESTClient, MainDm.RESTRequest, MainDm.RESTResponse, Url, Token);
    ...
    if IsDataArrayEmpty(resp) then begin
      try
        {Преобразуем весь JSON-ответ из UTF-8 в строку Delphi (обычно UTF-16)}
        JsonRes := System.UTF8ToString(RawByteString(resp));
        JsonResponse := TJSONObject.ParseJSONValue(JsonRes) as TJSONObject;

        pnt := '001';
        {получим все что в массиве data и начнем в цикле обработку этого блока}
        JResData := JsonResponse.GetValue('data') as TJSONArray;
        
        for i := 0 to JResData.Count-1 do begin
          spScriptWrite.Script.Text := 'SELECT RDB$SET_CONTEXT (''USER_SESSION'', ''EXT_APP'', ''1'') '+
          'FROM RDB$DATABASE;';
          ...
          pnt := '002';
          JItemData := JResData.Items[i] as TJSONObject;
          paymentsArr := '';

          {получим Id заказа их Base64. Его запишем дальше в REC_ID таблицы заказов}
          pnt := '003';
          orderId := BytesToString(DecodeBase64(JItemData.GetValue('id').Value));

          {В блоке Attributes лежат основные данные по заказу}
          pnt := '004';
          JItemAttributes := JItemData.Get('attributes').JsonValue as TJSONObject;
          
          {В блоке Attributes. OriginAddress лежит точка продажи. В тэге displayName. Для поиска точки в нашей
          системе, нам надо от туда вытащить значение}
          pnt := '005';
          JItemAttributesOriginAddress := JItemAttributes.Get('originAddress').JsonValue as TJSONObject;

          {В блоке Relationships нас интересует подблок "links" далее "related". Здесь на сколько я понял
          лежит ссылка на детали заказа. Вот нам надо его вытащить и дальше по этой ссылке дернуть еще раз сервис
          и далее уже крутиться в цикле по деталям заказа}
          pnt := '006';
          JItemRelationships := JItemData.Get('relationships').JsonValue as TJSONObject;
          
          {Извлекаем "related" из "relationships.entries.links"}
          pnt := '007';
          JItemRelEntries := JItemRelationships.Get('entries').JsonValue as TJSONObject;
          pnt := '008';
          JItemRelLinks := JItemRelEntries.Get('links').JsonValue as TJSONObject;
          pnt := '009';
          relatedLink := JItemRelLinks.Get('related').JsonValue.Value;

          {Если у нас RelatedLink не пустой, содержит сслыку на детали заказа, то вызовим по этой ссылке сервис
          дабы получить детали этого заказа. Если же пустой, то надо запись об этом в логе и выйти в целом из обработки}
          pnt := '010';
          if Length(relatedLink) > 0 then begin
            respOrdersDtl := GetFromKaspi(MainDm.RESTClient, MainDm.RESTRequest, MainDm.RESTResponse, relatedLink, Token);

            if Pos('Ошибка', respOrdersDtl) <> 0 then begin
              Log(respOrdersDtl);
              Result := respOrdersDtl;
              Exit;
            end;
          end else begin
            Result := 'Ссылка на детали заказа (data.relationships.entries.links) пустая. Обработка далее невозможна.';
            Log(Result);
            Exit;
          end;

          {Если детали заказа не пустые, т.е. json вернулся не пустой (data не пустая), то продолжим работу.
          Если json пустой (в data нет записей), то запишем об этом в логе и прекращаем обработку}
          pnt := '011';
          if IsDataArrayEmpty(respOrdersDtl) then begin
            {Преобразуем весь JSON-ответ из UTF-8 в строку Delphi (обычно UTF-16)}
            JsonResOrdersDtl := System.UTF8ToString(RawByteString(respOrdersDtl));
            JsonResponseOrdersDtl := TJSONObject.ParseJSONValue(JsonResOrdersDtl) as TJSONObject;
          end else begin
            Result := 'Детали заказа пустые. Обработка далее невозможна.';
            Log(Result);
            Exit;
          end;
          
          pnt := '012';
          {думаю здесь самое время записать дату в миллисекундах, что бы она для следующего вызова сервиса стала датой начала}
          GetSetFromToIni(1);
          pnt := '013';
          JResultOrdersDetails := JsonResponseOrdersDtl.GetValue('data') as TJSONArray;

          spScriptWrite.Script.Text := spScriptWrite.Script.tEXT + CRLF +
         'DELETE FROM KASPI_ORDERS_DTL_TMP;';

          for k := 0 to JResultOrdersDetails.Count-1 do begin
            JItemOrdersDetails := JResultOrdersDetails.Items[k] as TJSONObject;

            {В блоке Attributes лежат основные данные по деталям заказа}
            pnt := '014';
            JOrdersDtlAttributes := JItemOrdersDetails.Get('attributes').JsonValue as TJSONObject;

            {В блоке Attributes.offer нас интересует тэг code, в котором лежит значение для идентификации товара в нашей
            программе. Это из настроек идентификатор товара для Kaspi магазина и может быть: или ID товара в Beksar
            или Артикул товара или Штрихкод товара}
            pnt := '015';
            JOrdersDtlAttributesOffer := JOrdersDtlAttributes.Get('offer').JsonValue as TJSONObject;
            FOrdersDtlAttributesOfferCode := JOrdersDtlAttributesOffer.GetValue('code').Value;

            pnt := '016';
            JOrdersDtlRelationships := JItemOrdersDetails.Get('relationships').JsonValue as TJSONObject;

            summTovar := StrToFloat(JOrdersDtlAttributes.GetValue('quantity').Value) * StrToFloat(JOrdersDtlAttributes.GetValue('basePrice').Value);

            spScriptWrite.Script.tEXT := spScriptWrite.Script.tEXT + CRLF +
              'INSERT INTO KASPI_ORDERS_DTL_TMP (ZAKAZ_DETAILS, IDSKU_VALUE, AMOUNT, PRICE, SUMM, DISCOUNT_PERC, DISCOUNT_TENGE, DISCOUNT_SUMM, NDS, NDS_SUMM, COST_PRICE) ' +
                ' VALUES (GEN_ID(GEN_DETAILS_TMP, 1),' +
                '''' + FOrdersDtlAttributesOfferCode + '''' + ',' +
                JOrdersDtlAttributes.GetValue('quantity').Value + ',' +
                JOrdersDtlAttributes.GetValue('basePrice').Value + ',' +
                FloatToStr(summTovar) + ',0, 0, 0,' +
                FloatToStr(NdsValue) + ',' +
                FloatToStr(summTovar * NdsValue / (100 + NdsValue)) + ',' +
                JOrdersDtlAttributes.GetValue('basePrice').Value +
          ...
          resp := ExecScript(spScriptWrite);
          if resp <> '' then begin
            Log('Ошибка загрузки деталей заказа (EFIBError). Point = ' + pnt + '. ' + resp);
            exit;
          end;

          pnt := '018';
          with spProc do begin
            SQL.Text := 'EXECUTE PROCEDURE LOAD_ORDERS_FROM_KASPI (?TOCHKA_NAME_, ?Z_DATE_, ?Z_TIME_, ?G_CLIENT_, ' +
              '?G_PAYMENT_TYPE_, ?OSNOVANIE_, ?IS_SPISANIE_, ?NOTE_, ?SUMM_, ?SUMM_FACT_, ?SUMM_CASH_, ?SUMM_NONCASH_, ' +
              '?SUMM_BONUS_, ?SUMM_BONUS_GOT_, ?G_PRIHOD_, ?SUMM_PRIHOD_, ?BY_RECIPE_, ?IS_RESERVE_, ?PAYMENTS_, ?IS_MEBEL_, ' +
              '?DOSTAVSHIK_, ?DELIVERY_DATE_, ?ADDRESS_, ?PRICE_, ?UPLOAD_TO_EXT_APP_, ?G_OFFICIAL_, ?NOT_MINUS_OSTATOK_, ' +
              '?USER_ID_, ?LOG_BODY_, ?REC_ID_, ?IDSKU_)';
            StoredProcName := 'LOAD_ORDERS_FROM_KASPI';

            ParamByName('TOCHKA_NAME_').AsString := JItemAttributesOriginAddress.GetValue('displayName').Value;

            d := UnixTimeToDateTimeWithTimeZone(StrToInt64(JItemAttributes.GetValue('creationDate').Value));
            summCash := 0;
            summNonCash := StrToFloat(JItemAttributes.GetValue('totalPrice').Value);
            summBonus   := 0;
            ...
            ParamByName('NOTE_').AsString := 'Kaspi магазин';
            ParamByName('IS_SPISANIE_').AsInteger := 0;
            ParamByName('summ_').AsFloat := StrToFloat(JItemAttributes.GetValue('totalPrice').Value);
            ParamByName('summ_fact_').AsFloat := summCash + summNonCash;
            ParamByName('summ_cash_').AsFloat := summCash;
            ParamByName('summ_noncash_').AsFloat := summNonCash;
            ParamByName('SUMM_BONUS_').AsFloat := summBonus;
            ...
            pnt := '021';
            ParamByName('USER_ID_').AsInt64 := -2;
            ParamByName('G_OFFICIAL_').AsInt64 := -2;
            ParamByName('REC_ID_').AsInt64 := StrToInt64(orderId);
            //ParamByName('LOG_BODY_').AsString := FillLog;
            {Тип ключа для каспий магазина 0-Ключ Beksar, 1 - Артикул, 2 - баркод}
            ParamByName('IDSKU_').AsInteger := IDSKU;
            ...
            try
              ...
              if FieldIndex['ERR_CODE'] >= 0 then begin
                if (ParamByName('ERR_CODE').AsInteger <> 0) AND (ParamByName('ERR_CODE').AsInteger <> -900) then begin
                  ...
                  Log('Ошибка загрузки заказа. ID = ' + orderId + '. ' + err_str);
              ...
              on E: EFIBError do begin
                ...
                Log('Ошибка загрузки заказа. ID = ' + orderId + '. Point = ' + pnt + '. ' + err_str);
                Exit;
              end;
            end;
        ...
        on E: EFIBError do begin
          ...
          Log('Ошибка загрузки данных с Kaspi магазина (FIBError). ' + err_str);
        end;
        on E: Exception do begin
          tranMain.Rollback;
          Log('Ошибка загрузки данных с Kaspi магазина (Exception). ' + E.Message);
        end;
      end;
    end;

  FINALLY
    Log('Конец загрузки с Kaspi магазина.');
  END;
