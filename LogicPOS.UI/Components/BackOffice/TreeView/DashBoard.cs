    ...
    internal class DashBoard : GenericTreeViewXPO
    {
        ...
        public DashBoard(Window pSourceWindow, Entity pDefaultValue, CriteriaOperator pXpoCriteria, Type pDialogType, GenericTreeViewMode pGenericTreeViewMode = GenericTreeViewMode.Default, GenericTreeViewNavigatorMode pGenericTreeViewNavigatorMode = GenericTreeViewNavigatorMode.Default)
        {
            ...
            try
            {
                ...
                string themesRoot = PathsSettings.Paths["themes"].ToString();

                string candidate1 = string.Format("{0}Default/Backgrounds/Windows/LogicPOS_WorkFlow_{1}.png", themesRoot, cultureName);
                string candidate2 = string.Format("{0}Default/Backgrounds/Windows/Default/LogicPOS_WorkFlow_{1}.png", themesRoot, cultureName);
                string candidate3 = string.Format("{0}Default/Backgrounds/Windows/LogicPOS_WorkFlow_en-GB.png", themesRoot);
                string candidate4 = string.Format("{0}{1}", themesRoot, eventBackGround); // generic: Backgrounds/Windows/LogicPOS_WorkFlow.png

                string fileImageBack = null;
                if (System.IO.File.Exists(candidate1))
                {
                    fileImageBack = candidate1;
                }
                else if (System.IO.File.Exists(candidate2))
                {
                    fileImageBack = candidate2;
                }
                else if (System.IO.File.Exists(candidate3))
                {
                    fileImageBack = candidate3;
                }
                else if (System.IO.File.Exists(candidate4))
                {
                    fileImageBack = candidate4;
                }
                else
                {
                    // As a last resort do not set a background image
                    fileImageBack = string.Empty;
                }

                if (!string.IsNullOrEmpty(fileImageBack))
                {
