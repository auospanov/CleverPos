...
namespace logicpos
{
    internal class Utils
    {
        ...
        {
            // Проверяем, что GTK инициализирован
            // В GTK# нет Application.IsInitialized, поэтому просто проверяем что Init() был вызван
            try
            {
            ...
            {
                throw new InvalidOperationException("GTK Application не инициализирован. Вызовите Application.Init() перед созданием splash screen.");
            }

            string loadingImage = PathsSettings.ImagesFolderLocation + @"Other\working.gif";

            // Используем null как родительское окно для splash screen, если parent не инициализирован
            Window safeParent = parent;
            ...
            try
            {
                ...
                dialog.SetSizeRequest(400, 300); // Устанавливаем размер вместо Fullscreen
                dialog.Resizable = false;

                // Уничтожаем ActionArea только после настройки диалога
                dialog.ActionArea.Destroy();

                // Создаем контейнер для центрирования содержимого
                VBox mainVBox = new VBox(false, 10);
                mainVBox.SetSizeRequest(400, 300);
                ...
                labelBoot.Justify = Justification.Center;

                // Проверяем существование файла изображения перед созданием Image
                Gtk.Image imageWorking = null;
                if (File.Exists(loadingImage))
                {
                    try
                    {
                        imageWorking = new Gtk.Image(loadingImage);
                        imageWorking.SetSizeRequest(200, 200); // Ограничиваем размер изображения
                    }
                    catch (Exception ex)
                    {
                        // Логируем ошибку, но продолжаем без изображения
                        System.Diagnostics.Debug.WriteLine($"Ошибка загрузки изображения: {ex.Message}");
                    }
                }

                // Добавляем виджеты в контейнер с центрированием
                if (imageWorking != null)
                {
                    HBox imageContainer = new HBox(false, 0);
                    imageContainer.PackStart(new Label(""), true, false, 0); // Левая пустота
                    imageContainer.PackStart(imageWorking, false, false, 0);
                    imageContainer.PackStart(new Label(""), true, false, 0); // Правая пустота
                    mainVBox.PackStart(imageContainer, false, false, 10);
                }

                HBox labelContainer = new HBox(false, 0);
                labelContainer.PackStart(new Label(""), true, false, 0); // Левая пустота
                labelContainer.PackStart(labelBoot, false, false, 0);
                labelContainer.PackStart(new Label(""), true, false, 0); // Правая пустота
                mainVBox.PackStart(labelContainer, false, false, 10);

                // Добавляем пустое пространство снизу
                mainVBox.PackStart(new Label(""), true, false, 0);

                // Добавляем основной контейнер в диалог
                dialog.VBox.PackStart(mainVBox, true, true, 0);

                // Показываем виджеты по отдельности вместо ShowAll()
                mainVBox.Show();
                if (imageWorking != null)
                {
                    imageWorking.Show();
                ...
                // Показываем диалог в последнюю очередь
                dialog.Show();
            }
            catch (Exception ex)
            {
                // В случае ошибки уничтожаем диалог и пробрасываем исключение
                try
                {
                    dialog.Destroy();
                }
                ...
                throw new Exception($"Ошибка создания splash screen: {ex.Message}", ex);
            }
